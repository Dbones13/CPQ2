...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('Graphics_Migration_Execution_Year').GetValue())
    setAttrValue("Labor_Graphics_Migration_Message",'')
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        setAttrValue("Labor_Graphics_Migration_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("Graphics_Migration_Execution_Year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            container =  Product.GetContainerByName("MSID_Labor_Graphics_Migration_con")
            ...
            setAttrValue("Labor_Graphics_Migration_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("Graphics_Migration_Execution_Year",'')
else:
    year = int(Product.Attr('Graphics_Migration_Execution_Year').GetValue())
    setAttrValue("Labor_Graphics_Migration_Message",'')
