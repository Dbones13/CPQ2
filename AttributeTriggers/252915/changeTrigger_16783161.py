...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('3rd_Party_PLC_UOC_Labor_Execution_year').GetValue())
    setAttrValue("3rd_party_PLC_UOC_Labor_Message",'')
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        setAttrValue("3rd_party_PLC_UOC_Labor_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("3rd_Party_PLC_UOC_Labor_Execution_year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            container =  Product.GetContainerByName("3rd_Party_PLC_UOC_Labor")
            ...
            setAttrValue("3rd_party_PLC_UOC_Labor_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("3rd_Party_PLC_UOC_Labor_Execution_year",'')
else:
    year = int(Product.Attr('3rd_Party_PLC_UOC_Labor_Execution_year').GetValue())
    setAttrValue("3rd_party_PLC_UOC_Labor_Message",'')
