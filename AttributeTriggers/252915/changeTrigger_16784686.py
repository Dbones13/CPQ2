...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('Generic2_Execution_Year').GetValue())
    setAttrValue("Generic2_Message",'')
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        setAttrValue("Generic2_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("Generic2_Execution_Year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            container =  Product.GetContainerByName("MSID_Labor_Generic_System2_Cont")
            ...
            setAttrValue("Generic2_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("Generic2_Execution_Year",'')
else:
    year = int(Product.Attr('Generic2_Execution_Year').GetValue())
    setAttrValue("Generic2_Message",'')
