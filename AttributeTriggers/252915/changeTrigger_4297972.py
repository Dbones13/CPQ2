...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('EBR_Execution_Year').GetValue())
    setAttrValue("Labor_EBR_Message",'')
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        setAttrValue("Labor_EBR_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("EBR_Execution_Year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            container =  Product.GetContainerByName("MSID_Labor_EBR_Con")
            ...
            setAttrValue("Labor_EBR_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("EBR_Execution_Year",'')
else:
    year = int(Product.Attr('EBR_Execution_Year').GetValue())
    setAttrValue("Labor_EBR_Message",'')
