...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('TPS_TO_EXPERION_Execution_Year').GetValue())
    setAttrValue("Labor_TPS_TO_EXPERION_Message",'')
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        setAttrValue("Labor_TPS_TO_EXPERION_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("TPS_TO_EXPERION_Execution_Year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            ...
            setAttrValue("Labor_TPS_TO_EXPERION_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("TPS_TO_EXPERION_Execution_Year",'')
else:
    year = int(Product.Attr('TPS_TO_EXPERION_Execution_Year').GetValue())
    setAttrValue("Labor_TPS_TO_EXPERION_Message",'')
