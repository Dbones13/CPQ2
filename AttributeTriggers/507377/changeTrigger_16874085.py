...
tableLabor = SqlHelper.GetList('select * from INDUSTRIAL_SECURITY_LABOR_CUSTOM_TABLE')
			...
			if  new_row["Deliverable"]  ==  row.Deliverable:
				if gesLocation == "None" or gesLocation == '':
					...
					if gesLocation !="None":
						...
						new_row['FO Eng 2'] = row.FO_Eng_2_NoGES
	laborCont.Calculate()
	contList.append('IS_Labor_Container')
laborCont4 = Product.GetContainerByName('IS_Additional_Labor_Container')
if laborCont4.Rows.Count > 0:
	...
	contList.append('IS_Additional_Labor_Container')
