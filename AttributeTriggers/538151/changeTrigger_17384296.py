...
if productType == "New":
    try:
        ...
        if com_be_CS_CE < 0 :
            Product.Messages.Add('Contract End date cannot be earlier than - '+str(currnt_start_date))
            Product.Attr('ContractEndDate_EnabledService').AssignValue(prev_c_end_date)
        elif com_be_PE_CE > 0:
            Product.Messages.Add('Contract End date cannot be Later than - '+str(prev_c_end_date))
            Product.Attr('ContractEndDate_EnabledService').AssignValue(prev_c_end_date)

    except:
        Trace.Write('====Error Occured in Contract_EndDate Script======')