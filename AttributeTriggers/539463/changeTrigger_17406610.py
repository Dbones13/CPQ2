...
if laborCont.Rows.Count > 0:
    for new_row in laborCont.Rows:
        for row in tableLabor:
            if  new_row["Deliverable"]  ==  row.Deliverable:
                ...
                elif TagParserProduct.ParseString('<*CTX( Container(PCD Engineering Labor Container).Column(GES Eng).GetPermission )*>') == "Hidden":
                    Trace.Write("if")
    ...
    if gesLocation != "None":
        TagParserProduct.ParseString('<*CTX( Container(PCD Engineering Labor Container).Column(GES Eng).SetPermission(Editable) )*>')
