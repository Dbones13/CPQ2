gesLocation = TagParserProduct.ParseString('<* Value(PCD_Ges_Location_Labour) *>')
updateFlag = 0
...
if gesLocation <> "None":
    ...
    gesPerc = Product.Attr('PCD_GES_%').GetValue()

    for row in laborRows:
        gesPerc=float(gesPerc)
        excludedDeliverables = ['ControlEdge PCD Engineering Plan', 'ControlEdge PCD Procure Materials & Services', 'ControlEdge PCD Customer Training', 'ControlEdge PCD Project Close Out Report']
