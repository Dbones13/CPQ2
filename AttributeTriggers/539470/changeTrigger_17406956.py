...
tableLabor = SqlHelper.GetList('select Deliverable,Productivity,GES_Eng_1_No,GES_Eng,FO_Eng_1_NoGES,FO_Eng_1_GES_None,FO_Eng_1_GES,FO_Eng_2_NoGES,FO_Eng_2_GES_None,FO_Eng_2_GES,Rank,Execution_Country from DVM_ENGINEERING_LABOR')
if laborCont.Rows.Count > 0:
    for new_row in laborCont.Rows:
        for row in tableLabor:
            if  new_row["Deliverable"]  ==  row.Deliverable:
                ...
                elif TagParserProduct.ParseString('<*CTX( Container(DVM_Engineering_Labor_Container).Column(GES Eng).GetPermission )*>') == "Hidden":
                    new_row.GetColumnByName('FO Eng 1').SetAttributeValue(row.FO_Eng_1_NoGES)
                    new_row['FO Eng 1']=row.FO_Eng_1_NoGES
                    new_row.GetColumnByName('FO Eng 2').SetAttributeValue(row.FO_Eng_2_NoGES)
                    new_row['FO Eng 2']=row.FO_Eng_2_NoGES
    ...
    if gesLocation != "None" or gesLocation != "":
        TagParserProduct.ParseString('<*CTX( Container(DVM_Engineering_Labor_Container).Column(GES Eng).SetPermission(Editable) )*>')
