...
if laborCont.Rows.Count > 0:
    for new_row in laborCont.Rows:
        for row in tableLabor:
            if  new_row["Deliverable"]  ==  row.Deliverable:
                ...
                elif TagParserProduct.ParseString('<*CTX( Container(Experion HS Engineering Labor Container).Column(GES Eng).GetPermission )*>') == "Hidden":
    ...
    if gesLocation != "None" or gesLocation != "":
        TagParserProduct.ParseString('<*CTX( Container(Experion HS Engineering Labor Container).Column(GES Eng).SetPermission(Editable) )*>')
