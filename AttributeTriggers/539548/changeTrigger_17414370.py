...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('CB-EC_Upgrade_to_C300-UHIO_Execution_Year').GetValue())
    setAttrValue("Labor_CB-EC_Upgrade_to_C300-UHIO_Message",'')
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        setAttrValue("Labor_CB-EC_Upgrade_to_C300-UHIO_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("CB-EC_Upgrade_to_C300-UHIO_Execution_Year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            container =  Product.GetContainerByName("MSID_Labor_CB-EC_Upgrade_to_C300-UHIO_con")
            ...
            setAttrValue("Labor_CB-EC_Upgrade_to_C300-UHIO_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("CB-EC_Upgrade_to_C300-UHIO_Execution_Year",'')
else:
    year = int(Product.Attr('CB-EC_Upgrade_to_C300-UHIO_Execution_Year').GetValue())
    setAttrValue("Labor_CB-EC_Upgrade_to_C300-UHIO_Message",'')
