...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('FSC_to_SM_IO_audit_Execution_Year').GetValue())
    setAttrValue("Labor_FSCtoSM_IO_Audit_Message",'')
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        setAttrValue("Labor_FSCtoSM_IO_Audit_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("Generic1_Execution_Year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            container =  Product.GetContainerByName("MSID_Labor_FSC_to_SM_IO_Audit_Con")
            for row in container.Rows:
                if row["Deliverable"] not in ('Total','Off-Site','On-Site') and row.IsSelected:
                    row["Execution_Year"] = str(year)
                    #row.IsSelected = False
            setAttrValue("FSC_to_SM_IO_audit_Execution_Year",'')
            ScriptExecutor.Execute('PS_PopulateGESCost')
        else:
            setAttrValue("Labor_FSCtoSM_IO_Audit_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("FSC_to_SM_IO_audit_Execution_Year",'')
else:
    year = int(Product.Attr('FSC_to_SM_IO_audit_Execution_Year').GetValue())
    setAttrValue("Labor_FSCtoSM_IO_Audit_Message",'')
