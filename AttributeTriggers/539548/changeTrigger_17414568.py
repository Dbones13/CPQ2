# To Populate year For Labor deliverables tab Janhavi Tanna : CXCPQ-60159 :start 
...
if getCfValue("EGAP_Contract_Start_Date") != '' and getCfValue("EGAP_Contract_End_Date") != '':
    ...
    year = int(Product.Attr('ELEPIU_Execution_Year').GetValue())
    setAttrValue("Labor_ELEPIU_Message",'')
    Log.Write("Outside year condition1")
    if len(str(year)) < 4 or year < DateTime.Now.Year or year not in exeYearsRange:
        Log.Write("Inside year condition1")
        setAttrValue("Labor_ELEPIU_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("ELEPIU_Execution_Year",'')
    else:
        if year <= contractEndYear and year >= contractStartYear:
            container =  Product.GetContainerByName("MSID_Labor_ELEPIU_con")
            ...
            setAttrValue("Labor_ELEPIU_Message",'Execution Year should be in between the Contract start date and Contract end date')
            setAttrValue("ELEPIU_Execution_Year",'')
else:
    year = int(Product.Attr('ELEPIU_Execution_Year').GetValue())
    Log.Write("Inside else year condition2")
    setAttrValue("Labor_ELEPIU_Message",'')
    if year not in exeYearsRange:
        Log.Write("Inside if year condition2")
        setAttrValue("Labor_ELEPIU_Message",'User is allowed to enter current + next 3 years in 20XX format (e.g. 2023, 2025 etc)')
        setAttrValue("ELEPIU_Execution_Year",'')
    else:
        container =  Product.GetContainerByName("MSID_Labor_ELEPIU_con")
