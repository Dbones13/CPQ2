...
if Product.Attr('IP Security Tunnel Required').GetValue() == '':
    for row in activity.Rows:
        if row['Identifier'] == 'A10':
            activity.DeleteRow(row.RowIndex)
            break
else:
    item = SqlHelper.GetFirst("Select PartNumber, Activity,Identifier,Activity_Type,Rank FROM CT_ACTIVITIES where Product = 'MSS' and Identifier = 'A10'")
