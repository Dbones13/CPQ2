if Product.GetContainerByName('AR_HCI_SUBPRD').Rows.Count>1:
    ...
    fo_eng_ges = row.Product.GetContainerByName('HCI_PHD_GES_Eng').Rows.Clear()
    fo_eng_ges = row.Product.GetContainerByName('HCI_PHD_GES_Eng').AddNewRow(False)
    fo_eng_ges = row.Product.GetContainerByName('HCI_PHD_GES_Eng').Rows[0]
    fo_eng_ges.SetColumnValue('Engineer','GES Eng 1')
    ...
    fo_eng_ges['Participation'] = Product.Attributes.GetByName('AR_HCI_GES Participation %').GetValue()
