...
ScriptExecutor.Execute('PS_R2Q_HideContainerAttribute')'''

import R2QRMContainerRowAdded
R2QRMContainerRowAdded.cal_function(Product)
#ScriptExecutor.Execute('PS_R2Q_HideContainerAttribute')
if Product.GetContainerByName('SM_RG_Cabinet_Details_Cont_Left') and Product.GetContainerByName('SM_RG_Cabinet_Details_Cont_Left').Rows and Product.GetContainerByName('SM_RG_Cabinet_Details_Cont_Left').Rows.Count > 0 and Product.GetContainerByName('SM_RG_Cabinet_Details_Cont_Left').Rows[0].GetColumnByName('Marshalling_Option').DisplayValue in ['Universal Marshalling', 'Universal_Marshalling']:
    TagParserProduct.ParseString('<*CTX( Container(SM_RG_Cabinet_Details_Cont_Left).Column(SM_RG_RelayTypeForESD).SetPermission(Hidden) )*>')