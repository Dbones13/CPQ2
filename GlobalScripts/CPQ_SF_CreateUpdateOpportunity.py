...
def main():
    ...
    if opportunityId:
        ...
        if CL_GeneralIntegrationSettings.ALL_REV_ATTACHED_TO_SAME_OPPORTUNITY:
            ...
            if not quoteResp:
                soql =  """?q=SELECT+ID,+NAME+FROM+QUOTE+WHERE+OpportunityID='{OppID}'+AND+Quote_ID__c={quoteId}+AND+Owner_ID__c={userId}"""
                apiResponse = class_sf_integration_modules.call_soql_api(class_sf_integration_modules.get_authorization_header(adminToken), soql.format(OppID = opportunityId, quoteId = Quote.QuoteId, userId=Quote.UserId))
                if JsonHelper.Serialize(apiResponse).Contains("records") and apiResponse["totalSize"] > 0:
                    quoteResp = {"Id": str(apiResponse["records"][0]["Id"])}
        if linkedQuotesResp:
            linkedQuotes = filter(lambda resp: resp[CL_SalesforceQuoteParams.SF_QUOTE_ID_FIELD] != Quote.QuoteId 
                                  or resp[CL_SalesforceQuoteParams.SF_OWNER_ID_FIELD] != Quote.UserId, linkedQuotesResp["body"]["records"])
        ...
        if linkedQuotes is not None:
            for linkedQuote in linkedQuotes:
                if linkedQuote[CL_SalesforceQuoteParams.SF_PRIMARY_QUOTE_FIELD]:
                    record = dict()
        ...
        if compositePayload:
            ...
            permissionList = [class_sf_integration_modules.build_permission_checklist(CL_SalesforceIntegrationParams.SF_OPPORTUNITY_OBJECT, True, True),
                              class_sf_integration_modules.build_permission_checklist(CL_SalesforceQuoteParams.SF_QUOTE_OBJECT, True, True, True)]
            # Call REST API
    ...
    #############################################
    quoteItems = [{"item": item, "lookUps": list(), "sfId": "", "sfStandardPriceBookEntryId": "", "sfCustomPriceBookEntryId": ""} for item in Quote.Items if item.ProductTypeName not in CL_GeneralIntegrationSettings.PRODUCT_TYPE_EXCLUSION]
    quoteItems = None
    if quoteItems:
