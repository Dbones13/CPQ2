...
def is_action_allowed(Quote, User, externalParameters, actionId):
    ...
    ##Added to fix the issue for CXCPQ-80437 - Lahu  ####
    ScriptExecutor.ExecuteGlobal("GS_ca_expirequote")
    ##############################################
...
###############################################################################################
# Function to attach additional parameters to Quote
###############################################################################################
def set_quote_multiple_params(Quote, externalParameters):
    external_to_custom = {
        "msid": "MSID",
        "contactid": "ContactId",
        "egapproposaltype": "EGAP_Proposal_Type",
        "entitlementselectedvalue":"Entitlement"
    }
    for field, param in external_to_custom.items():
            if field in Quote.CustomFields:
                if param in externalParameters:
                    Quote.GetCustomField(field).Content = str(externalParameters[param])
