...
class CL_SalesforceIntegrationModules(CL_CpqHelper):
    ...
    def call_rest_api(self, url, headers, body, method, integrationReference, permissionList = None):
        ...
        try:
            if method == API.GET:
                ...
                if "'IsR2QRequest__c': 'Yes'" in str(body) and "Approval_Req_from_Booking_Gatekeeper__c" in str(body):
                    #qno=TagParserQuote.ParseString('<*CTX( Quote.CartCompositeNumber )*>')
                    #s=0
                    #for i in qno:
                    # 	s=s+int(i)
                    #if User.UserName!='H587668':
                    #ticks = DateTime.UtcNow.Ticks
                    #nanoseconds = ticks
                    #nanoseconds2 = ticks/2
                    # Use modulo to limit the range (e.g., 0 to 99)
                    #random_number = (nanoseconds % 100)+(nanoseconds2 % 100)+20
                    ticks = DateTime.UtcNow.Ticks
                    ticks = ticks % 100
                    random_number = (ticks%30) if ticks>30 else ticks
                    Log.Info("RandomNo=>"+str(random_number))
                    time.sleep(random_number)
                    response = RestClient.Post(url, body, headers)
                    if '"statusCode": "UNABLE_TO_LOCK_ROW"' in str(response):
                        ticks = DateTime.UtcNow.Ticks
                        ticks = ticks % 100
                        random_number = (ticks%30) if ticks>30 else ticks
                        Log.Info("RandomNo2=>"+str(random_number))
                        time.sleep(random_number)
                        response = RestClient.Post(url, body, headers)
                        if '"statusCode": "UNABLE_TO_LOCK_ROW"' in str(response):
                            ticks = DateTime.UtcNow.Ticks
                            ticks = ticks % 100
                            random_number = (ticks%30) if ticks>30 else ticks
                            Log.Info("RandomNo3=>"+str(random_number))
                            time.sleep(random_number)
                            response = RestClient.Post(url, body, headers)
                else:
                    response = RestClient.Post(url, body, headers)
    ...
    def build_cr_record_create_quote(self, opportunityId):
        record = quote_integration_mapping(self.Quote, self.TagParserQuote, True) # Custom flag added to change modified by user
    ...
    def build_cr_record_update_quote(self, sfQuoteId):
        record = quote_integration_mapping(self.Quote, self.TagParserQuote, False) # Custom flag added to change modified by user
        record = self.get_key_quote_mappings(record)
        record["Id"] = sfQuoteId
        record["attributes"] = {"type": CL_SalesforceQuoteParams.SF_QUOTE_OBJECT}
        return record
    ...
    def check_api_permissions(self, permissionList):
        ...
        for permission in permissionList:
            ...
            if sObjectPermission:
                ...
                for action in ["createable", "updateable", "queryable"]:
