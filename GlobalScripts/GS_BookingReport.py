...
def getProductLineMap(groupingKey):
	...
	for item in Quote.Items:
		guidPartNumberMap[item.QuoteItemGuid] = item.PartNumber if item.ProductName not in ('Winest Labor Import') else item.ProductName
		if item[groupingKey].Value == "":
			continue
		plDict[item.PartNumber] = item[groupingKey].Value
		productType = item.ProductTypeName if item.ProductName != 'TPC_Product' else item.QI_ProjectType.Value
		...
		ptData = ptPlMap.get(productType, {"totalCost": 0, "totalSell": 0})
...
def populateQuoteTable(quote, ptPlMap, srvModulePriceMap, sespBgpDict,plDict):
	...
	global totalChinaTP2
	currencyFactor = getCurrencyFactor(quote)
	totalQuoteSellPrice, totalQuoteCostPrice = 0.00, 0.00
	...
	for header in headerOrder:
		...
		for pl, plData in data.items():
			if pl == "totalCost" or pl == "totalSell":
				continue
			newRow = table.AddNewRow()
			newRow["Product_Line"] = pl
