    ...
    if containerRows.Count > 0:
        for i, containerRow in enumerate(containerRows):
            if ((attrName in skip_conditions and i in skip_conditions[attrName]) or (attrName in SMskip_conditions and i in SMskip_conditions[attrName]) or (attrName in CCRskip_conditions and i in CCRskip_conditions[attrName])):
                continue
            containerRowDict = {col.Name: containerRow[col.Name] for col in containerRow.Columns if col.Name not in {'IO_Type_with_hint', 'Deliverable_w_hint', 'IO_Type_Icon', 'Part_Summary', 'Displays/Shapes/Faceplates_hint'}}
...
def container_functions(product,Quotecontaxt):
    Quotecontaxt.SetGlobal('normalcompare', '')
    selectAttributedict = {}
    ...
    getsysupval = sorted(getsysupval, key=lambda item: get_sort_index(item, Quotecontaxt))
    ...
    Quotecontaxt.SetGlobal('normalcompare', str(updated_data))

def Experion_system_update(Product,Experion_dict):
    domain_required = '1' if Experion_dict.get('Domain_Controller_Required', 'No') == 'Yes' else '0'
    Product.Attr('No of Domain Controller').AssignValue(domain_required)

    firewall_qty = Experion_dict.get('Network_Firewall_Required', '0')
    Product.Attr('No of Firewall, Routers, Simulators, TOR').AssignValue(firewall_qty)

    opc_required = Experion_dict.get('Opc_server_required', '') == 'Yes'
    opc_redundant = Experion_dict.get('OPC_server_redundancy_required', '') == 'Redundant'
    opc_qty = '2' if opc_required and opc_redundant else ('1' if opc_required else '0')
    Product.Attr('No of OPC Server').AssignValue(opc_qty)

    Product.Attr('No of EMDB Servers').AssignValue('0')
    Product.Attr('No of Data highway').AssignValue('0')
    Product.Attr('Experion_R2Q_flag').AssignValue('R2q_product')
    thin_clients = Experion_dict.get('CMS Flex Station Qty 0_60', '0')
    Product.Attr('Experion_sys_No_of_Thin_clients').AssignValue(thin_clients)

def cost_plsg_lob_upd(Quote):
