...
def getEHPMHARTIOLabourHours(Product):
    ...
    for key in parameters:
        if key == "MSID_CommonQuestions":
            ...
            var_2 = getRowData(Product,key,parameters[key]["var_2"])
            var_12 = getRowData(Product,key,parameters[key]["var_12"])
            var_4 = getRowData(Product,key,parameters[key]["var_4"])
            var_9 = getRowData(Product,key,parameters[key]["var_9"])
            var_13 = getRowData(Product,key,parameters[key]["var_13"])
...
def getC200MigrationLabourHours(Product):
    ...
    if migrationScenario == 'C200 to ControlEdge UOC':
        try:
            ...
            c200PartCont = Product.GetContainerByName('MSID_C200_Migration_Added_Parts_Common_Container').Rows
        ...
        for key in parameters:
            ...
            if key == "MSID_CommonQuestions":
                for row in getContainer(Product,key).Rows:
                    var_13 =  getRowData(Product,key,parameters[key]["var_13"]) if getRowData(Product,key,parameters[key]["var_13"]) else 'Yes'
                    var_14 = getRowData(Product,key,parameters[key]["var_14"]) if getRowData(Product,key,parameters[key]["var_14"]) else 'Yes'

            if key == "C200_Services_2_Cont":
                for row in getContainer(Product,key).Rows:
                    var_18 =  getRowData(Product,key,parameters[key]["var_18"]) if getRowData(Product,key,parameters[key]["var_18"]) else 'No'
                    var_15 = getRowData(Product,key,parameters[key]["var_15"]) if getRowData(Product,key,parameters[key]["var_15"]) else 'No'
        ...
        for key in parameters:
            ...
            if key == "MSID_CommonQuestions":
                for row in getContainer(Product,key).Rows:
                    var_19 =  getRowData(Product,key,parameters[key]["var_19"]) if getRowData(Product,key,parameters[key]["var_19"]) else 'Yes'
                    var_21 = getRowData(Product,key,parameters[key]["var_21"]) if getRowData(Product,key,parameters[key]["var_21"]) else 'Yes'

            if key == "C200_Services_2_Cont":
                for row in getContainer(Product,key).Rows:
                    var_24 =  getRowData(Product,key,parameters[key]["var_24"]) if getRowData(Product,key,parameters[key]["var_24"]) else 'No'
                    var_20 = getRowData(Product,key,parameters[key]["var_20"]) if getRowData(Product,key,parameters[key]["var_20"]) else 'No'
...
def getCBECLabourHours(Product):
    ...
    parameters = {"CB_EC_migration_to_C300_UHIO_Configuration_Cont":{"var_1":"CB_EC_How_many_CBs_are_being_migrated","var_2":"CB_EC_How_many_ECs_are_being_migrated","var_4":"CB_EC_Do_you_want_Series_C_RAM_Battery_Backup"},"CB_EC_Services_1_Cont":{"var_16":"CB_EC_Total_Number_of_Aux_function","var_15":"CB_EC_Total_Number_of_Complex_Loop","var_14":"CB_EC_Total_Number_of_Cascade_Loop","var_13":"CB_EC_Total_Number_of_Digital_Composite_points_HGDCP","var_12":"CB_EC_Total_Number_of_Digital_Output_points_HGDOT","var_11":"CB_EC_Total_Number_of_Digital_Input_points_HGDIN","var_10":"CB_EC_Total_Number_of_Regulatory_points_HGREG","var_9":"CB_EC_Total_Number_of_Analog_Output_points_HGAOT","var_8":"CB_EC_Total_Number_of_Analog_Input_points_HGAIN","var_3":"CB_EC_Number_of_CB_EC_Power_Supplies_that_need_to_be_replaced","var_7":"CB_EC_Is_CB_EC_interfacing_with_DHEB_on_the_system","var_18":"CB_EC_Does_the_Customer_have_all_updated_ILDs","var_19":"CB_EC_Does_Customer_want _to_validate_ILDs_by_Honeywell"},"CB_EC_Services_2_Cont":{"var_6":"CB_EC_Is_common_PV_sharing_between_two_or_more_than_two_CB_EC","var_20":"CB_EC_Detailed_DDS_required","var_22":"CB_EC_Is_HotCutover_required","var_5":"CB_EC_Will_Honeywell_perform_equipment_installation_activities","var_17":"CB_EC_Data_Gathering_required"},"MSID_CommonQuestions":{"var_21":"MSID_Acceptance_Test_Required"}}
    ...
    for key in parameters:
        ...
        if key == "CB_EC_Services_1_Cont":
            ...
            var_16 = getFloat(getRowData(Product,key,parameters[key]["var_16"]))
        ...
        if key == "CB_EC_Services_2_Cont":
            ...
            var_6 = getRowData(Product,key,parameters[key]["var_6"])
            var_17 = getRowData(Product,key,parameters[key]["var_17"])
            var_20 = getRowData(Product,key,parameters[key]["var_20"])
            var_22 = getRowData(Product,key,parameters[key]["var_22"])
...
def getxPMtoC300LabourHours(Product):
    try:
        CC_ZHR041 = 0
        CC_C8SS01 = 0
        CC_C8DS01 = 0
        ...
        CC_TCF901 = 0
        _8939_HN = 0
        xpmPartCont = Product.GetContainerByName('MSID_xPM_C300_Added_Parts_Common_Container').Rows
    ...
    for key in parameters:
        ...
        if key =="xPM_C300_Services_Cont":
            ...
            var_37 = getRowData(Product,key,parameters[key]["var_37"])
