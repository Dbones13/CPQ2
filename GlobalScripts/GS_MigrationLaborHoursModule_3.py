...
def getFSCTOSMLabourHours(Product):
    ...
    parameters ={"FSC_to_SM_Services":{"var_7":"FSC_to_SM_In_Office_Eng_hours_per_Audit_Report","var_10_t":"FSC_to_SM_Has_the_System_Audit_been_performed","var_9":"FSC_to_SM_Factory_Acceptance_Test_required","var_6":"FSC_to_SM_ACAD","var_8":"FSC_to_SM_On_Site_Eng_hours_per_Audit_Report"},"FSC_to_SM_Configuration":{"var_1":"FSC_to_SM_How_many_systems_with_same_configuration_to_be_migrated_in_this_proposal","var_2_YN":"FSC_to_SM_Are_the_FSCs_in_this_configuration_connected_to_a_SafeNet_network","var_2_1":"FSC_to_SM_How_many_FSC_Systems_are_in_the_SafeNet_network","var_2_2":"FSC_to_SM_How_many_FSC_Systems_from_the_SafeNet_network_are_we_migrating_in_the_first_phase"}}
    ...
    for key in parameters:
        if key == "FSC_to_SM_Services":
            ...
            var_10_t = getRowData(Product,key,parameters[key]["var_10_t"])
...
def getFDMLabourHours(Product):
    
    parameters ={"FDM_Upgrade_General_questions":{"var_UP_ADD_CONF_YN":"FDM_Upgrade_Additional_Components_to_be_offered_for_number_of_FDMs_?"}, "FDM_Upgrade_Configuration":{"var_2_YN":"FDM_Upgrade_Do_you_want_to_upgrade_this_FDM","var_3_1":"FDM_Upgrade_Total_FDM_Clients","var_4_1":"FDM_Upgrade_Total_RCIs_including_Experion_PKS_Server_Interfaces"}, "FDM_Upgrade_Services":{"var_5":"FDM_Upgrade_Number_of_Experion/TPS_Server_interface","var_6":"FDM_Upgrade_Number_of_Server_Network_Interface_Licenses","var_7":"FDM_Upgrade_Number_of_Exp_Stations_w/FDM_Maintenance_Station_View_features","var_8":"FDM_Upgrade_Number_of_FDM_Gateways","var_11":"FDM_Upgrade_Site_Acceptance_Test"}, "FDM_Upgrade_Additional_Configuration":{"var_12":"FDM_Upgrade_Number_of_PVST_Planner_Licenses_for_supported_HART_ESD_Devices","var_3_2":"FDM_Upgrade_Number_of_FDM_Clients","var_4_2":"FDM_Upgrade_Number_of_remote_PCs_connecting_to_FDM_Serve_ via_LAN","var_14_YN":"FDM_Upgrade_Are_additional_components_required_for_this_FDM"}}
    var_2 = 0
    var_14 = 0
    var_UP_ADD_CONF_YN = getRowData(Product,"FDM_Upgrade_General_questions","FDM_Upgrade_Additional_Components_to_be_offered_for_number_of_FDMs_?")
    ...
    for key in parameters:
        if key == "FDM_Upgrade_Configuration":
            ...
            var_4_1 = 0
            for row in getContainer(Product, key).Rows:
                if row[parameters[key]["var_2_YN"]] == "Yes":
                    var_2 =var_2 +1
                    var_3_1 += getFloat(row[parameters[key]["var_3_1"]])
                    var_4_1 += getFloat(row[parameters[key]["var_4_1"]])
        if key == "FDM_Upgrade_Additional_Configuration":
            ...
            if var_UP_ADD_CONF_YN != "No":
                for row in getContainer(Product, key).Rows:
                    if row[parameters[key]["var_14_YN"]] == "Yes":
                        var_14 =var_14 +1
                        var_3_2 += getFloat(row[parameters[key]["var_3_2"]])
                        var_4_2 += getFloat(row[parameters[key]["var_4_2"]])
        ...
        if key == "FDM_Upgrade_Services":
            ...
            var_7 = getFloat(getRowData(Product,key,parameters[key]["var_7"]))
            var_8 = getFloat(getRowData(Product,key,parameters[key]["var_8"]))
            var_11 = getRowData(Product,key,parameters[key]["var_11"])

    var_3 = var_3_1 + var_3_2
    var_4 = var_4_1 + var_4_2
    ...
    migration_of_rci = 0
    Installation_and_Configuration_of_Network_Interfaces = ((var_6*30.00)/60.00) if(var_6>15.00) else ((var_6*30.00)/60.00)
    ...
    if var_8<=2.00:
        ...
        Installation_and_Configuration_of_FDM_Gateways = 13.00 + (var_8-4.00)*2.5
    else:
...
def getLMtoELMMLabourHours(Quote,Product):
    try:
        #var 2
        _900A01_0202 = 0
        _900B01_0301 = 0
        ...
        LMPartCont = Product.GetContainerByName('MSID_LM_TO_ELMM_Added_Parts_Common_Container').Rows
        LM3rdparty = Product.GetContainerByName('MSID_Third_Party_Added_Parts_Common_Container').Rows
    ...
    var_1 = getFloat(getRowData(Product,"LM_to_ELMM_ControlEdge_PLC_Migration_Gen_Cont","LM_Qty_Of_LM_Pair_To_Be_Migrated"))
    ...
    for key in parameters:
        ...
        if key == "LM_to_ELMM_Services":
            ...
            var_24 = getRowData(Product,key,parameters[key]["var_24"])
...
def getFSCtoSMIOAuditLabourHours(Product):
    ...
    row = Cont1.Rows[0]
    var_1 = int(row['FSC_to_SM_IO_Migration_Total_FSC_SM_Systems']) if row['FSC_to_SM_IO_Migration_Total_FSC_SM_Systems'] != '' else 0
    service = Cont3.Rows[0]
    var_9 = service['FSC_to_SM_IO_Has_the_FSC_IO_Audit_been_performed']
