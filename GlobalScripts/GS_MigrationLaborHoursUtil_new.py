...
def getTotalEngHours(Product,container):
	...
	Trace.Write("totalFinalHours"+str(totalFinalHours))
...
def getProjectMangementHours(Product):
	...
	totalVirtHours = getTotalEngHours(Product,"MSID_Labor_Virtualization_con") if "Virtualization System Migration" in selectedProducts else 0
	...
	totalGS2Hours = getTotalEngHours(Product,"MSID_Labor_Generic_System1_Cont") if "Generic System 2" in selectedProducts else 0
	totalGS3Hours = getTotalEngHours(Product,"MSID_Labor_Generic_System1_Cont") if "Generic System 3" in selectedProducts else 0
	totalGS4Hours = getTotalEngHours(Product,"MSID_Labor_Generic_System1_Cont") if "Generic System 4" in selectedProducts else 0
	totalGS5Hours = getTotalEngHours(Product,"MSID_Labor_Generic_System1_Cont") if "Generic System 5" in selectedProducts else 0
	...
	if ("FDM Upgrade 1" in selectedProducts) or ("FDM Upgrade 2" in selectedProducts) or ("FDM Upgrade 3" in selectedProducts):
		totalfdmupgradeHours = getTotalEngHours(Product,"MSID_Labor_FDM_Upgrade_Con")
	else:
		totalfdmupgradeHours = 0
	EngHours = totalOpmFinalHours + totalLcnFinalHours + totalEbrFinalHours + totalElcnFinalHours + totalOrionConsoleHours + totalEHPMHours + totalTPSHours + totalTCMIHours + totalEHPMHARTIOHours + totalC200Hours + totalcbecHours + totalxPMHours + totalfsctosmHours + totalfdmupgradeHours + totallmHours + totalfsctosmauditHours + totalXP10Hours + totalGraphicsHours + totalCDActuatorHours + totalfscsmioHours + totalCWSHours + totalplcuocHours + totalVirtHours + totalQCSHours  + totalGS1Hours + totalGS2Hours + totalGS3Hours + totalGS4Hours + totalGS5Hours + totalTPAHours + totalELEPIUHours
	Trace.Write("EngHours"+str(EngHours)+"ebrfinalhrs"+str(totalEbrFinalHours)+"totalC200Hours"+str(totalC200Hours))
	pmOtherActivities = 0
	if EngHours > 0:
