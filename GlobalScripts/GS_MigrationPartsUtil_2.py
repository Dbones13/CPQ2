...
def updateAttrDictWithCustomC200toC300(product, attrValDict, Quote):
    ...
    for row in con2.Rows:
        ...
        """Buying UHO Kit"""
        if (FIM2Points == 0) and (UHOCondition <= 64) and (cabinetTypeCustomerPlans != 'New Front Access Only Series C Cabinet' and cabinetTypeCustomerPlans != 'New Front & Rear Access Series C Cabinet') and ((X_TKFTEB01 <= 7 and (controllerRedundancy == 'Redundant' or controllerRedundancy == '')) or (X_TKFTEB01 <= 6 and (controllerRedundancy == 'Non Redundant'))):
            if epks_rel in ["R520","R530"] and noOf1765DIDOAIAO == 0 and SeriesAIORacks == 0:
                X_CCZHR042 += 1
                X_CCZHMT10 = X_CCZHR042
                X_TCCSUG50 = X_CCZHR042 if sespType == 'No' else 0
                X_TSCSUG50 = X_CCZHR042 if sespType == 'Yes' else 0
                ...
                X_TCCSUG50 = X_CCZHR041 if sespType == 'No' else 0
                X_TSCSUG50 = X_CCZHR041 if sespType == 'Yes' else 0
    ...
    attrValDict['Y_CU_PWPR20'] = Y_CU_PWPR20
    attrValDict['Y_CU_PWPN20'] = Y_CU_PWPN20
...
def populateWriteInsOPM(product):
    i = 0
    j= 0
    curr_exp_rel = ['R430.x','R431.x','R432.x','R500.x','R501.x','R510.x','R511']
    writeInData = dict()
    msid= product.Attr('Migration_MSID_Choices').GetValue()
    sysNumber= product.Attr('Migration_MSID_System_Number').GetValue()
    area = msid +" - "+ sysNumber
    ...
    for row in con1.Rows:
        if row["MSID_Current_Experion_Release"] in curr_exp_rel:
            i = 1    
    con2 = getContainer(product, "OPM_Basic_Information")
    for row in con2.Rows:
        if row["OPM_Is_this_is_a_Remote_Migration_Service_RMS"] == "No" and row["OPM_If_AMT_Will_Not_Be_Used"] != "Yes":
            j = 1
