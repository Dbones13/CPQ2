...
def updateAttributeWithLMtoELMMValues(product, attrValDict):
	...
	ot = product.Attr('LM_to_ELMM_ControlEdge_PLC_Operating_temperature').GetValue()
	prvdfetcable = product.Attr('ATT_LM_ELMM_HONEYWELL_PROVIDE_FTE').GetValue()
	cablenplc = product.Attr('LM_to_ELMM Cable Length').GetValue()
		...
		if ot == "0 to 60 DegC or Marine application":
			...
			if prvdfetcable in ["Yes"] :
				...
				if lmPLCCont.Rows[index]['LM_average_Cable_length_for_IO_network_connection'] == '2m':
					if prvdfetcable in ["Yes"] and lmPLCCont.Rows[index]['LM_select_IO_network_topology'] == 'Star':
						LM51305482_102 = math.ceil(getFloat(attrValDict["900SP1-0200"][-1]) + 2.0 + LM51305482_102)
					if prvdfetcable in ["Yes"] and lmPLCCont.Rows[index]['LM_select_IO_network_topology'] == 'Ring':
						LM51305482_102 = math.ceil(getFloat(attrValDict["900SP1-0200"][-1]) + 1.0 + LM51305482_102)
					if prvdfetcable in ["Yes"] and lmPLCCont.Rows[index]['LM_select_type_of_Switch_for_the_IO_network'] == 'Multimode Redundant':
						LM51305482_202 = math.ceil(getFloat(attrValDict["900SP1-0200"][-1]) + 2.0 + LM51305482_202)
					if prvdfetcable in ["Yes"] and lmPLCCont.Rows[index]['LM_select_type_of_Switch_for_the_IO_network'] == 'Single Mode Redundant':
						LM51305482_202 = math.ceil(getFloat(attrValDict["900SP1-0200"][-1]) + 2.0 + LM51305482_202)
