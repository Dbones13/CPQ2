...
def FSCtoSM_IOparts(product, attrValDict, parentProduct=None):
	cont1 = product.GetContainerByName("FSC_to_SM_IO_Series_1_&_2_FSC_IO_configurations")
	if parentProduct:
		scope_check = parentProduct.Attr('MIgration_Scope_Choices').GetValue()
	else:
	...
	if scope_check != "LABOR":
		...
		IO_place = 0
		if contGen and contGen.Rows.Count:
			if scope == 1:
			...
			if scope == 1:
				IO_check = product.Attr("FSC_to_SM_IO_Migration_Where_will_IO_be_installed").GetValue()
				if IO_check == "Other cabinet":
					IO_place = 1
