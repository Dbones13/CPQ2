def populateUOCData(Quote):
    ...
    hierarchy = {'Level0': "New / Expansion Project", 'Level1': "System Group", "Level2": "ControlEdge UOC System", "Level3":"UOC Control Group"}
    ...
    for row in QT_Table.Rows:
        ...
        for Item in Quote.Items:
            ...
            if row['System_Item_GUID'] == Item.QuoteItemGuid and Item.ProductName == "ControlEdge UOC System" and row['CG_Name'] == '':
                Sysdic = {}
                for attr in Item.SelectedAttributes:
                    error_flag=False
            ...
            elif row['CG_ITEM_GUID'] == Item.QuoteItemGuid and Item.ProductName == "UOC Control Group":
                CGdic = {}
                for attr in Item.SelectedAttributes:
                    error_flag=False
                    if attr.Name not in lst and "PLC" not in attr.Name:
                        ...
                        for column in attr_containers:
                            if column.Name in cg_columns:
                                if column.Name == 'UOC_Integrated_Marshalling_Cabinet':
                                    ...
                                    CGdic[column.Name] = column.Value
                                elif column.Name in ['UOC_AO_100_250', 'UOC_AO_250_499', 'UOC_AO_500']:
                                    LIO_CNT += int(column.Value)
                                    UOC_AO += int(column.Value)
                                    CGdic['UOC_AO'] = UOC_AO
                                elif column.Name in ['UOC_AO_HART_100_250', 'UOC_AO_HART_250_499', 'UOC_AO_HART_500']:
                                    LIO_CNT += int(column.Value)
                                    UOC_AO_HART += int(column.Value)
                                    ...
                                    LIO_CNT += int(column.Value)
                                    UOC_Universal += int(column.Value)
                                    CGdic['UOC_Universal'] = UOC_Universal
                                elif column.Name in ['UOC_DO_10_250', 'UOC_DO_250_500']:
                                    LIO_CNT += int(column.Value)
                                    UOC_DO += int(column.Value)
                                    CGdic['UOC_DO'] = UOC_DO
                                elif column.Name in ['UOC_Digital_Input16_120240VAC', 'UOC_Digital_Input16_125VDC']:
                                    LIO_CNT += int(column.Value)
                                    UOC_DI += int(column.Value)
                                    CGdic['UOC_DI'] = UOC_DI
