...
def populateCotainer(foPartNumberCon,entry,costWithConversion,PriceData):
    ...
    Trace.Write("________ FO_Part_Number    =  "+str(entry.FO_Part_Number) +" Product_Module =  " +str(entry.Product_Module) +"Cost =   "+ str(row["Cost"] ))
...
def populateGesMaterials(foPartNumberCon,entry,gesRegionalCost,PriceData):
    ...
    Trace.Write("________GES_Part_Number    =  "+str(entry.GES_Part_Number) +" Product_Module =  " +str(entry.Product_Module) +"Cost =   "+ str(row["Cost"] ))
    row["ListPrice"] = str((PriceData.get(entry.GES_Part_Number,'')))


def populateFOPartNumberCon(foPartNumberCon,mpaAvailable,activeServiceContract,executionYear):
    partsList = []
    foPartNumberCon.Rows.Clear()
    LOB = Quote.GetCustomField("Booking LOB").Content #Get Quote Booking LOB
    queryData = SqlHelper.GetList("select TOP 2000 fo.*,p.PRODUCT_NAME from Labor_FO_Part_Number fo join products p on p.PRODUCT_CATALOG_CODE = fo.FO_Part_Number where fo.LOB = '"+LOB+"' and fo.GES_Part_Number = ''")
    if len(queryData) :
    ...
    queryData1 = SqlHelper.GetList("select fo.*,p.PRODUCT_NAME from Labor_FO_Part_Number fo join products p on p.PRODUCT_CATALOG_CODE = fo.GES_Part_Number where fo.GES_Part_Number != '' and fo.LOB = '"+LOB+"' and fo.GES_Location = '{}'".format(Product.Attr('MSID_GES_Location').SelectedValue.ValueCode))
    if len(queryData1) :
