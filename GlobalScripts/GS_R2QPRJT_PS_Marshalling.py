...
#Log.Info('GS_R2QPRJT_PS_MarshallingResponse Param-->>'+JsonHelper.Serialize(Param))
...
def WirteinItemsCreation(row, installedSpareIO, iospare, NISAI, NISAO, NISDI, NISDO, ISAI, ISAO, ISDI, ISDO, RelayDI, RelayDO, execountry, relayesd= '', SMproductline = ''):
	...
	final_request_body=("{{'quoteNumber' : '{0}', 'parentConfig' : '', 'config' : '{1}', 'module' : 'Marshalling', 'executionCountry': '{16}', 'cabinetType' : '{15}', 'productLine' : '{18}', 'installedSpareIO' : '{3}', 'IOSpare' : '{4}', 'RelayTypeForESD' : '{17}', 'NISAI' : '{5}', 'NISAO' : '{6}', 'NISDI' : '{7}', 'NISDO' : '{8}', 'ISAI' : '{9}', 'ISAO' : '{10}', 'ISDI' : '{11}', 'ISDO' : '{12}', 'RelayDI' : '{13}', 'RelayDO' : '{14}'}}").format(Quote.CompositeNumber, controlgroup.decode("utf-8"), '', installedSpareIO, iospare, NISAI, NISAO, NISDI, NISDO, ISAI, ISAO, ISDI, ISDO, RelayDI, RelayDO, cabinetType, execountry, relayesd, productLine)
	...
	#Log.Write('Response for Marshalling from Apigee 1== '+ str(res))
	res2=JsonHelper.Deserialize(res)
	#Log.Write('Response for Marshalling from Apigee 22== '+ str(res2))
	if res2 and len(res2) > 0 and 'Failure' not in res2:
		...
		for itemdet in res2:
			...
			if itemdet['Extended Description'].startswith('TS8 Cabinet Base'):
				if 'C300' in Param.ActionName.split('_')[0]:
				...
				if 'R2Q Safety Manager ESD' in Param.ActionName.split('_')[0] and productLine =='ESD':
		...
		if any(marshalling_cabinet_counts.values()):
			for systemCont in Product.GetContainerByName('R2Q CE_System_Cont').Rows:
				...
				if systemCont['Selected_Products'] in ("R2Q Safety Manager ESD", "R2Q Safety Manager FGS"):
					if systemCont['Selected_Products'] == Param.ActionName.split('_')[0]:
		...
		if WriteinProdCont.Rows.Count > 0:
...
try:
																	...
																	if ciotype in IoContainerlist and len(row[ciotype]) > 0 and all(row[ciotype]):
																		for index in range(len(row[ciotype][0])):
																			...
																			for k, v in item.items():
																				...
																				if k in RelayDOlist:
																					sums['RelayDO'] += v
																WirteinItemsCreation(row, CGinstalledSpareIO, CGIoSpare, sums['NISAI'], sums['NISAO'], sums['NISDI'], sums['NISDO'], sums['ISAI'], sums['ISAO'], sums['ISDI'], sums['ISDO'], sums['RelayDI'], sums['RelayDO'], executionCountry)
						#safety manager
													...
													for SMcontDetails in data[key][1]:
														...
														if SMcontDetails['SM_CG_Name'] == Param.ActionName.split('_')[1]:
															...
															if SmMarshallingvalue:
																...
																WirteinItemsCreation(SMcontDetails, SmIoInsatlledSpare, SmCabinetspare, SM_sums['SMNISAI'], SM_sums['SMNISAO'], SM_sums['SMNISDI'], SM_sums['SMNISDO'], SM_sums['SMNISAIB'], SM_sums['SMNISAOB'], SM_sums['SMNISDIB'], SM_sums['SMNISDOB'], SM_sums['SMRelayDI'], SM_sums['SMRelayDO'], executionCountry, relayesd, SMproductline)


	Log.Info('GS_R2QPRJT_PS_Marshalling Success-->>')
	final_request_body={'QuoteNumber':str(Param.QuoteNumber),'CartId':str(Param.CartId),'RevisionNumber': str(Param.RevisionNumber),'UserName':str(User.UserName),'Module':'New/Expansion','Action':'Update','Status':'Success','Action_List':[{'ActionName':str(Param.ActionName),'ScriptName':'GS_R2QPRJT_PS_Marshalling'}]}
	res = RestClient.Post(excel_Url, RestClient.SerializeToJson(str(final_request_body)),header)
	#res2=JsonHelper.Deserialize(res)
	Log.Write('Response for Marshalling from Apigee == '+ str(res))
	...
	final_request_body={'QuoteNumber':str(Param.QuoteNumber),'CartId':str(Param.CartId),'RevisionNumber': str(Param.RevisionNumber),'UserName':str(User.UserName),'Module':'New/Expansion','Action':'Update','Status':'Fail','Action_List':[{'ActionName':str(Param.ActionName),'ScriptName':'GS_R2QPRJT_PS_Marshalling','ErrorMessage':str(ex)}]}
