...
def add_part(cont, parts_to_add, Product):
    for part in parts_to_add:
        try:
            ...
            row.GetColumnByName("CE_Part_Qty").Value = str(int(parts_to_add[part]['Quantity']))
            row.GetColumnByName("CE_Part_Description").Value = (str(parts_to_add[part]['Description']))
            row.GetColumnByName("CE_Final_Quantity").ReferencingAttribute.AssignValue(str(int(parts_to_add[part]['Quantity'])))
...
def update_qty(cont, parts_to_update):
    for part in parts_to_update: #dictionary where key is index, and value is new qty
        ...
        cont.Rows[part].GetColumnByName("CE_Part_Qty").Value = str(quantity)
        adjQuantity = adjQuantity if adjQuantity else 0
        finalQuantity = quantity + int(adjQuantity)
        cont.Rows[part].GetColumnByName("CE_Final_Quantity").ReferencingAttribute.AssignValue(str(finalQuantity))
    ...
    for part in parts_dict:
        ...
        for row in current_parts:
            if row[1] == part: #comparing part numbers
                part_found = True
                if parts_dict[part]['Quantity'] == 0: #This leaves it in the current_parts list, which will ensure the part gets deleted
                    break
                if (row[2] != str(int(parts_dict[part]['Quantity']))): #comparing qtys
                    parts_to_update[row[0]] = parts_dict[part]['Quantity']
                current_parts.remove(row) #this is within the if condition so that if qty = 0, the part is deleted.
                break
        if part_found == False and parts_dict[part]['Quantity'] != 0:
            parts_to_add[part] = {}
            parts_to_add[part]['Quantity'] = parts_dict[part]['Quantity']
            parts_to_add[part]['Description'] = parts_dict[part]['Description']
    return parts_to_add, parts_to_update, current_parts

def execute(Product, container_name, parts_dict):
    ...
    update_qty(cont, parts_to_update)
