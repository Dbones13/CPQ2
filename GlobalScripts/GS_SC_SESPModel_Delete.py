...
if a=="New":
    sespmodel=Product.GetContainerByName("SC_Models_Scope")
m = []
msid = []
for row in sespmodel.Rows:
    if row.IsSelected:
        m.append(int(row['HiddenRowIndex']) if a == "Renewal" else row.RowIndex)
        if a == "Renewal" and row['MSIDs'] not in msid:
            msid.append(row['MSIDs'])
m.reverse()
for i in m:
    sespmodel.DeleteRow(i)
    if a=="Renewal":
...
if a=="Renewal":
    ...
    ScriptExecutor.Execute('PS_SESPModel_Errors')
otu = Product.Attr('One time upgrade').SelectedValue
if otu:
    ScriptExecutor.Execute('GS_MSIDs_OTU_SESP') # OTU
