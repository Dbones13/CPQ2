...
class IOComponents:
    ...
    #calculation for components SUMDIOR
    def getUniversalIOCountRedNIS(self):
        SUMDIOR1 = 0
        questions = []
        columns = ''
        if self.Product.Name == "SM Control Group":
        ...
        if len(questions):
            for qn in questions:
                ...
                for column_name in columns:
                    ...
                    SUMDIOR1 += inputed_value
        return SUMDIOR1

    #Intermediate calculation for the component SUMDIOR for nmr/sil2/sil3
        ...
        if len(questions):
            for qn in questions:
                ...
                try:
                    ...
                    row_index = self.getRowIndex(container, key_column_name, qn)
                    for column_name in columns:
                        SUMDIOR3 += self.getColumnValue(container, row_index, column_name)
                        #Trace.Write("{}={}".format(column_name,SUMDIOR3))
                except Exception as e:
                    Trace.Write("{} is may not be visible".format(container_name))
                    Trace.Write(str(e))
        return SUMDIOR3

    def get_Sumdior(self):
        SUMDIOR3 = 0
        questions = []
        column_name = ''
        if self.Product.Name == "SM Control Group":
            if self.Product.GetContainerByName('SM_CG_Cabinet_Details_Cont_Left').Rows[0].GetColumnByName('Marshalling_Option').DisplayValue=="Universal Marshalling":
                try:
                ...
                if di_do_relay == 'Yes' or namur == 'Yes':
                    ...
                    SUMDIOR3 += self.getSilRelyNamur(['SDO(1) 24Vdc 500mA DIO (0-5000)'], ['Red_SIL2_RLY'])
				    #Trace.Write(SUMDIOR3)
				    #Red (SIL 3 RLY) type IOs
                    SUMDIOR3 += self.getSilRelyNamur(['SDI(1) 24Vdc DIO (0-5000)', 'SDO(1) 24Vdc 500mA DIO (0-5000)'], ['Red_SIL3_RLY'])
				    #Trace.Write(SUMDIOR3)
				    #Red NMR type IOs
                    SUMDIOR3 += self.getSilRelyNamur(['SDI(1) 24Vdc DIO (0-5000)'], ['Red_NMR'])
				    #Trace.Write(SUMDIOR3)
				    #Red NMR Safety type IOs
                    SUMDIOR3 += self.getSilRelyNamur(['SDI(1) 24Vdc DIO (0-5000)'], ['Red_NMR_Safety'])
            ...
            if self.Product.GetContainerByName('SM_RG_ATEX Compliance_and_Enclosure_Type_Cont').Rows[0].GetColumnByName('Enclosure_Type').DisplayValue=="Cabinet":
                ...
                if self.Product.GetContainerByName('SM_RG_Cabinet_Details_Cont_Left').Rows[0].GetColumnByName('Marshalling_Option').DisplayValue=="Universal Marshalling":
                    try:
    ...
    def getUniversalIOCountRedRLY(self):
        SUMDIOR2 = 0
        questions1 = []
        columns1 = ''
        if self.Product.Name == "SM Control Group":
        ...
        if len(questions1):
            for qn in questions1:
                ...
                for column_name in columns1:
                    inputed_value = self.getColumnValue(container, row_index, column_name)
                    #Trace.Write("{}={}".format(column_name,inputed_value))
                    SUMDIOR2 += inputed_value
        return SUMDIOR2

    def SUMDIOR_value(self):
        SUMDIOR=0
        if self.Product.Name == "SM Control Group":
            
            sum1 = self.getUniversalIOCountRedRLY()
            sum2 = self.getUniversalIOCountRedNIS()
            sum3 = self.get_Sumdior()
            sumf = sum2 + sum1 + sum3
            installed_spare =self.getPercentageSpareSpace()
            SUMDIOR = d.Ceiling(sumf * (1 + installed_spare))
        if self.Product.Name == "SM Remote Group":
            if self.Product.Attr("SM_Universal_IOTA_Type").GetValue()=="PUIO":
                ...
                sum3 = self.get_Sumdior()
                sumf = sum2 + sum1 + sum3
                Trace.Write("sumf= "+str(sumf))
                installed_spare =self.getPercentageSpareSpace()
                SUMDIOR = d.Ceiling(sumf * (1 + installed_spare))
                Trace.Write("SUMDIORTot= "+str(SUMDIOR))
        return SUMDIOR
'''atr=IOComponents(Product)
var=atr.getPercentageSpareSpace()
Trace.Write("Percentage_Spare_Space= "+str(var))
sum1=atr.getUniversalIOCountRedRLY()
Trace.Write("sum1= "+str(sum1))
sum2=atr.getUniversalIOCountRedNIS()
Trace.Write("sum2= "+str(sum2))
sum3=atr.get_Sumdior()
Trace.Write("sum3= "+str(sum3))
red=atr.SUMDIOR_value()
Trace.Write("SUMDIOR= "+str(red))'''