...
def getValue(Product,Col_name):
    return Product.Attr(Col_name).GetValue()
...
def getgraphicsMigrationUI(Product):
    parameters ={"Graphics_Migration_Migration_Scenario":{"var_26":"Graphics_Migration_Type_of_Existing_Displays","var_21":"Graphics_Migration_Is_the_system_connected_to","var_16":"Migration_Graphics_Using_device_control_digital","var_17":"Migration_Graphics_Have_multiple_DI_or_DO","var_18":"Graphics_Migration_Require_multi_tag_shapes","var_19":"Graphics_Migration_Have_process_module_point","var_31":"Migration_Graphics_Have_array_point_that_requires","var_32":"Migration_Graphics_Willing_to_accept_alternative","var_51":"ATT_GMPERUSD","var_34":"Graphics_Migration_Have_specific_native_or_GUS","var_33":"Graphics_Migration_Require_an_HMI_interface_for_AM"},"Graphics_Migration_Displays_Shapes_Faceplates":{"var_2":"Total_Number_of_Displays","var_35":"Experion_shapes_multiplier"}}
    for key in parameters:
        if key == "Graphics_Migration_Migration_Scenario":
            var_26_check = getValue(Product,parameters[key]["var_26"])
            var_21 = getValue(Product,parameters[key]["var_21"])
            var_16 = getValue(Product,parameters[key]["var_16"])
            var_17 = getValue(Product,parameters[key]["var_17"])
            var_18 = getValue(Product,parameters[key]["var_18"])
            var_19 = getValue(Product,parameters[key]["var_19"])
            var_31 = getValue(Product,parameters[key]["var_31"])
            var_32 = getValue(Product,parameters[key]["var_32"])
            var_33 = getValue(Product,parameters[key]["var_33"])
            var_34 = getValue(Product,parameters[key]["var_34"])
            var_51 = getFloat(getValue(Product,parameters[key]["var_51"]))
            Trace.Write("var_51"+str(var_51))
    ...
    #Attr_check = Product.Attr("ATT_GMSCENARIO")
    #row_check = container_check.Rows[0]
    check = Product.Attr("Graphics_Migration_Select_Vertical_Market").GetValue()
