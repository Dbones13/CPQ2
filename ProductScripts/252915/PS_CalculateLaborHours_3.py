from GS_MigrationLaborHoursModule import getnumberOfjumpRealease,checkForMPACustomer,calculateTotals
from GS_MigrationLaborHoursModule_1 import getDataGatheringHours,getDOcumentationHours,getPreFATSAT,getMigration2Hours,getSATHours,getMigrationL1Hours,getPostMigrationTask,getOpmMcoe,getOPMDeploymentL2
		...
		for row in opmEngineeringCon.Rows:
			...
			if row["Deliverable"] not in ('Off-Site','On-Site','Total'):
				if row["Calculated_Hrs"] not in ('',"0.00"):
					...
					row["Adjustment_Productivity"] = reCalAdj(row,oldCalHrs)
					row["Final_Hrs"] = calculateFinalHours1(row,oldCalHrs)
	calculateEMPEfforts(Product,opmEngineeringCon)
