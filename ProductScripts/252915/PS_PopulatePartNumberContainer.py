...
def populateFOPartNumberCon(foPartNumberCon,mpaAvailable,activeServiceContract,executionYear,SelectedProducts):
    ...
    if queryData1 is not None:
        ...
        #Trace.Write("######################"+str(gesRegionalCost.get("SVC_GES_P350B_CN",'')))
        if ('QCS RAE Upgrade' in Product.Attr('MSID_Selected_Products').GetValue() or 'CWS RAE Upgrade' in Product.Attr('MSID_Selected_Products').GetValue())   and (Product.Attr('MSID_GES_Location').GetValue() != 'GES China' and Product.Attr('MSID_GES_Location').GetValue() != 'None'):
...
Log.Write(str(Product.Name))
executionYear = str(getDefaultExecutionYear())
SelectedProducts = "'"+Product.Attr("MSID_Selected_Products").GetValue().replace("<br>","','").replace("LCN One Time Upgrade","LCN").replace("FDM Upgrade 1","FDM Upgrade") + "','PM','PA'"
