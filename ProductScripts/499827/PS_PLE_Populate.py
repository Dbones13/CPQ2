...
def sortRow(cont,rank,new_row_index):
    ...
    while sort_needed == True:
        #Trace.Write("PLE newrow rank of previous row: {0}, new_row_index: {1}".format(cont.Rows[new_row_index-1].GetColumnByName('Rank').Value, new_row_index))
        #Trace.Write("PLE index cal ---->"+str(new_row_index-1))
        if int(new_row_index)>0 and int(cont.Rows[new_row_index-1].GetColumnByName('Rank').Value) > int(rank):
...
Trace.Write("Project_Categorization= "+str(process_type))
Trace.Write("Does= "+str(Does))
if process_type == "Hardware only Project":
    disallow_lst.append("LE Internal Kickoff Meeting")
    disallow_lst.append("LE External Kickoff Meeting")
    disallow_lst.append("LE Document and Deliverable List")
    disallow_lst.append("LE Internal Test and FAT Procedure")
if process_type == "Hardware only Project" or process_type == "Small Project":
    disallow_lst.append("LE Contract Review Document")
    disallow_lst.append("LE Gate 1 Review")
    disallow_lst.append("LE Engineering Plan")
    disallow_lst.append("LE Deliverable Control Index")
    disallow_lst.append("LE Gate 4 Review")
if Does == "No" or Does == "" and (process_type == "" or process_type == "Hardware only Project" or process_type == "Small Project" or process_type == "Medium Large Project"):
    disallow_lst.append("LE Site Acceptance Test and Sign Off")
...
if New_Expansion =="" or New_Expansion == "Expansion":
    if CutOver == "No" or CutOver == "":
        disallow_lst.append("LE Cut Over Procedure")
if New_Expansion =="" or New_Expansion == "Expansion":
...
laborCont = Product.GetContainerByName('PLE_Labor_Container')
tableLabor = SqlHelper.GetList('select  Deliverable,Productivity,GES_Eng_1_No,GES_Eng,FO_Eng_1_NoGES,FO_Eng_1_GES_None,FO_Eng_1_GES,FO_Eng_2_NoGES,FO_Eng_2_GES_None,FO_Eng_2_GES,Rank,Calculated_Hrs,Execution_Country from CT_PLE_Labor_Container')
...
for row in tableLabor:
    if row.Deliverable not in disallow_lst and row.Deliverable not in current_deliverables:
        ...
        Trace.Write("newrow-----"+str(new_row.RowIndex))
        new_row["Deliverable"] = row.Deliverable
        Trace.Write("deliverable"+new_row["Deliverable"])
        ...
        if row.FO_Eng_1_NoGES:
            Trace.Write("foengineering"+row.FO_Eng_1_NoGES+"deliverable"+row.Deliverable)
        ...
        if gesLocation == "None" or gesLocation == '':
            ...
            new_row.GetColumnByName('GES Eng').ReferencingAttribute.SelectDisplayValue('SYS GES Eng-BO-'+gesLocation)
            ...
            if row.Deliverable == cont_row.GetColumnByName('Deliverable').Value:
                ...
                if cont_row['GES Location'] != gesLocation:
                    ...
                    if gesLocation == "None" or gesLocation == '':
                        ...
                        cont_row.GetColumnByName('GES Eng').ReferencingAttribute.SelectDisplayValue('SYS GES Eng-BO-'+gesLocation)
                    applyChanges = 1
