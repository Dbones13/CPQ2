import datetime
#PS_Project_Management_Labor_Container_Populate
def isFloat(val):
...
System_Group = Product.GetContainerByName('CE_SystemGroup_Cont').Rows
CutOver = 0
for sys in System_Group:
    prod = sys.Product
    CutOver =  prod.Attr('CE_Cutover').GetValue()
process_type = 'None'
if Product.GetContainerByName("CE_Project_Questions_Cont").Rows.Count > 0:
    process_type = Product.GetContainerByName('CE_Project_Questions_Cont').Rows[0].GetColumnByName('Project Categorization').Value
Does = 'No'
gesLocation = 'None'
if Product.GetContainerByName('Labor_Details_New/Expansion_Cont').Rows.Count > 0:
    Does = Product.GetContainerByName('Labor_Details_New/Expansion_Cont').Rows[0].GetColumnByName('Labor_Site_Activities').Value
    gesLocation = Product.GetContainerByName('Labor_Details_New/Expansion_Cont').Rows[0].GetColumnByName('GES_Location').Value
#process_type1 = Product.GetContainerByName('CE_General_Inputs_Cont').Rows[0].GetColumnByName('CE_Cutover').Value
New_Expansion = ''
if Product.GetContainerByName('CE_SystemGroup_Cont').Rows.Count > 0:
    New_Expansion = Product.GetContainerByName('CE_SystemGroup_Cont').Rows[0].GetColumnByName('New_Expansion').Value
Trace.Write("New_Expansion= "+str(New_Expansion))

if process_type == "Hardware only Project":
    disallow_lst.append("PM Document and Deliverable List")
    disallow_lst.append("PM Factory Acceptance Test and Sign Off")
    disallow_lst.append("PM Acceptance Protocol")
    disallow_lst.append("PM Punch List and Clearance Plan")
    disallow_lst.append("PM Hand Over Protocol to Service")

if process_type == "Hardware only Project" or process_type == "Small Project":
    disallow_lst.append("PM Gate 1 Review")
    disallow_lst.append("PM Project Control Plan")
    disallow_lst.append("PM Internal Procurement Plan")
    disallow_lst.append("PM Health and Safety Plan")
    disallow_lst.append("PM Work Breakdown Structure")
    disallow_lst.append("PM Site Acceptance Test and Sign Off")
    disallow_lst.append("PM As Built Documents Review")
    disallow_lst.append("PM Gate 4 Review")
if Does == "No" or Does == "" and (process_type == "" or process_type == "Hardware only Project" or process_type == "Small Project" or process_type == "Medium Large Project"):
    disallow_lst.append("PM Site Acceptance Test and Sign Off")
if Does == "Yes" and (process_type == "Hardware only Project" or process_type == "Small Project"):
    disallow_lst.append("PM Site Acceptance Test and Sign Off")

if New_Expansion =="" or New_Expansion == "Expansion":
    if CutOver == "No" or CutOver == "":
        disallow_lst.append("PM Cut Over Procedure")
if New_Expansion =="" or New_Expansion == "Expansion":
    if CutOver == "Yes" and (process_type == "Hardware only Project" or process_type == "Small Project"):
        disallow_lst.append("PM Cut Over Procedure")
else:
    disallow_lst.append("PM Cut Over Procedure")
...
tableLabor = SqlHelper.GetList('select Deliverable,Productivity,GES_Eng_1_No,GES_Eng,FO_Eng_1_NoGES,FO_Eng_1_GES_None,FO_Eng_1_GES,FO_Eng_2_NoGES,FO_Eng_2_GES_None,FO_Eng_2_GES,Rank,Calculated_Hrs,Execution_Country from PROJECT_MANAGEMENT_LABOR_UI')
...
laborAdditionCustomCont = Product.GetContainerByName('PM_Additional_Custom_Deliverables_Labor_Container')
for row in laborAdditionCustomCont.Rows:
    if row['Execution Country'] == '':
        row['Execution Country'] = query.Execution_County
    if row['Execution Year'] == '':
        if Quote.GetCustomField('R2QFlag').Content == "Yes":
            row["Execution Year"] = Quote.GetCustomField('R2Q_PRJT_Execution_Year').Content
        else:
            row['Execution Year'] = str(contract_start)
        row.Product.Attr('Additional_Project_FOENG_Deliverables').SelectDisplayValue('SYS LE-Lead Eng')
    if row['GES Location'] != gesLocation:
        row.SetColumnValue('GES Location', gesLocation)
        row.GetColumnByName('GES Location').ReferencingAttribute.AssignValue(gesLocation)
        if gesLocation == "None" or gesLocation == '':
            row.GetColumnByName('GES Eng').ReferencingAttribute.SelectDisplayValue('None')
        else:
            row.GetColumnByName('GES Eng').ReferencingAttribute.SelectDisplayValue('SYS GES PM-BO-'+gesLocation)
        row.ApplyProductChanges()
        row.Calculate()
laborAdditionCustomCont.Calculate()
