...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for IS_Labor_Container Labor Container
    deliverables = Product.GetContainerByName('IS_Labor_Container').Rows
    gesLocation = Product.Attr("IS_GES_Location").GetValue()
    count = count1 = count2 = 0
    Labor_Message = Labor_Message2 = Labor_Message3 = ColumnName = ""
    ...
    for row in deliverables:
        ...
        if (finalHrs not in ('','0','0.0') and (FO_Eng_2_val == '' or FO_Eng_2_val == 'None') and FO2_Eng > 0 and FO2_Eng <= 100):
            count1 = count1 + 1
            ColumnName = "Fo Eng 2"
            Labor_Message2 = Labor_Message2 + " - " +str(deliverable.Value) + "<br>"
        elif (finalHrs not in ('','0','0.0') and (FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
            count1 = count1 + 1
            ColumnName = "Fo Eng 1"
            Labor_Message2 = Labor_Message2 + " - " +str(deliverable.Value) + "<br>"
    ...
    if count1 > 0:
        Labor_Message2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the {}% Split to 0% <br>'.format(ColumnName)+"</b>" + Labor_Message2
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message2
        else:
            Labor_Message = Labor_Message2
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in Industrial Security (Access Control). Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for IS_Additional_Labor_Container
    IS_Error_Message_val = IS_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('IS_Additional_Labor_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            IS_Error_Message_val = IS_Error_Message_val + " - " +str(deliverable) + "<br>"
        ...
        if (finalHrs not in ('','0','0.0') and (FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
            count1 = count1 + 1
            ColumnName = "Fo Eng"
            IS_Error_Message_val2 = IS_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    """
    if count > 0:
        IS_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + IS_Error_Message_val
        if Labor_Message != "":
            Labor_Message += "<br>" + IS_Error_Message_val
        else:
            Labor_Message = IS_Error_Message_val"""

    if count1 > 0:
        IS_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the {}% Split to 0% <br>'+"</b>" + IS_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + IS_Error_Message_val2
        else:
            Labor_Message = IS_Error_Message_val2
    if count2 > 0:
