import datetime
#PS_FGC_Engineering_Labor_Container_Populate
...
disallow_lst = []
FGC_attr_value = Product.Attr('FGC_Cause_and_Effect_Matrix_Required?').GetValue()
if FGC_attr_value == "No":
    disallow_lst.append("F&G Cause and Effect Chart")
FGC_attr_value = Product.Attr('FGC_2D_Fire _and_Gas_Plot_Plan_Layouts_Req').GetValue()
if FGC_attr_value == "No":
    disallow_lst.append("F&G Plot Plan (2D Layouts)")
FGC_attr_value = Product.Attr('FGC_ISA_Type_Datasheets_Required_?').GetValue()
if FGC_attr_value == "No":
    disallow_lst.append("F&G Datasheets")
FGC_attr_value = Product.Attr('FGC_Gas_Dispersion_Modeling_Required?').GetValue()
if FGC_attr_value == "No":
    disallow_lst.append("F&G Hazardous Consequence Modeling (HCM)")
FGC_attr_value = Product.Attr('FGC_Site_Survey_Required?').GetValue()
if FGC_attr_value == "No":
    disallow_lst.append("F&G Site Study with Site Survey Report")
...
gesMapping = {'GES India':'IN','None':'None','GES Egypt':'EG'}
		...
		for cont_row in laborCont.Rows:
			if row.Deliverable == cont_row.GetColumnByName('Deliverable').Value:
				...
				if cont_row['GES Location'] != gesLocationVC:
					...
					if gesLocation == "None" or gesLocation == '':
						cont_row["GES Eng % Split"]= row.GES_Eng_1_No
						...
						if updateEngSplit:
							cont_row["GES Eng % Split"]= row.GES_Eng
...
laborAddi = Product.GetContainerByName('FGC_Additional_Labour_Container').Rows
...
for row1 in laborAddi:
	...
	row1.Calculate()
Product.GetContainerByName('FGC_Additional_Labour_Container').Calculate()
