...
Trace.Write("outside the if condition")

if (('Labor Deliverables' in tabs or 'General Input' in tabs) and scope == 'HW/SW + LABOR'):
    Trace.Write("inside the if condition")
    
    #Error Message for   PAGA Labor Container
    deliverables = Product.GetContainerByName('PAGA_Labor_Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in Variodyne Public Address General Alarm Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for PAGA_Additional_Labour_Container
    Safety_Manager_Error_Message_val = Safety_Manager_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('PAGA_Additional_Labour_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    ...
    if count1 > 0:
        Safety_Manager_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the {}% Split to 0% <br>'.format(ColumnName)+"</b>" + Safety_Manager_Error_Message_val2
        if Labor_Message != "":
