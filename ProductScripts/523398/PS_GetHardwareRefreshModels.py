...
for rows in contdetails.Rows:
	if rows["Type"] == "Renewal" and rows["Module"] == "Hardware Refresh" and rows.Product.Attr('SC_Product_Type').GetValue() == 'Renewal':
		py_discount1= 0
		comparisonCont = rows.Product.GetContainerByName('ComparisonSummary')
		...
		discount_percent=0
		HRHW_Cont = rows.Product.GetContainerByName('SC_ModelsHiddenSS_HR_RWL')
		setDefaultValues = SqlHelper.GetList("select ServiceProduct, Entitlement from CT_SC_ENTITLEMENTS_DATA where IsMandatory = 'True' and ServiceProduct = '{}'".format(rows.Product.Name))
		ENT_SS_CONT = rows.Product.GetContainerByName('SC_ServicePrdEntSS_HR_RWL')
		ENT_SS_CONT.Calculate()
		MODELS_CONT_SS_Hidden = rows.Product.GetContainerByName('SC_ModelsHiddenSS_HR_RWL')
		MODELS_SSE_CONT = rows.Product.GetContainerByName('SC_ValidModels_HR_RWL')
		MODELS_CONT_SS_Hidden.Clear()
		for row in MODELS_SSE_CONT.Rows:
			Trace.Write("value testing")
			...
			if rows.Product.Attr('SC_Pricing_Escalation').GetValue()=="No":
			...
			if rows.Product.Attr('SC_Pricing_Escalation').GetValue() == "Yes":
		...
		SP_Discount = {}
		HRHW_Cont = rows.Product.GetContainerByName('SC_ModelsHiddenSS_HR_RWL')
		ComparisonSummary = rows.Product.GetContainerByName("ComparisonSummary")
		...
		sscont= rows.Product.GetContainerByName('SC_ModelsSS_HR_RWL')
