...
cont = Product.GetContainerByName("CBM_Models_Cont")
cont1 =  Product.GetContainerByName('CBM_Pricing_Container')
discount = 0
for row in comparision_cont.Rows:
    if row.IsSelected:
        for row1 in cont1.Rows:
            row1['CY_Count'] = '0'
            row1['CY_ListPrice'] = '0'
        for row2 in cont.Rows:
            row2['CY_LevelOffering'] = '0'
            row2['CY_PMCBM'] = '0'
            row2['CY_ListPrice'] = '0'
            if row2['PY_ListPrice'] == "":
                row2['PY_ListPrice'] = 0
            if float(row2['PY_ListPrice']) > float(row2['CY_ListPrice']):
                row2['Comments'] = "Scope Reduction"
            elif float(row2['PY_ListPrice']) < float(row2['CY_ListPrice']):
                row2['Comments'] = "Scope Addition"
            elif float(row2['PY_ListPrice']) == float(row2['CY_ListPrice']):
                row2['Comments'] = "No Scope Change"
        Product.Attr('SC_ScopeRemoval_Check').AssignValue('0')
        comparision_cont.Calculate()
        cont1.Calculate()
    else:
        if Product.Attr('SC_ScopeRemoval_Check').GetValue() == '0':
            list1=[]
            list2=[]
            if 1:
            ...
            for i in dicta.keys():
                if i not in lista:
                    ...
                    rown['PY_PMCBM'] = PMCBM
                    rown['PY_ListPrice'] = str(float(dicta.get(i)))
            cont1.Calculate
            ...
            for row1 in cont1.Rows:
                ...
                cont1.Calculate()
            Product.Attr('SC_ScopeRemoval_Check').AssignValue('1')
cont = Product.GetContainerByName("CBM_Models_Cont")
for row in comparision_cont.Rows:
