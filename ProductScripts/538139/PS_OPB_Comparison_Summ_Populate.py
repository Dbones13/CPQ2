...
if Product_type == 'Renewal': #and Quote_Number in [None,''] and Contract_Number not in [None, '']
    ...
    if Model_Scope.Rows.Count:
        for row in Model_Scope.Rows:
            SP_listPrice[row["Service_Product"]] = SP_listPrice.get(row["Service_Product"],0) + float(row["Previous_Year_List_Price"])
    ...
    if Comp_summ.Rows.Count:
        for row in Comp_summ.Rows:
            ...
            discount_percent = float(row['PY_Discount_SFDC']) if row['PY_Discount_SFDC'].strip() != '' else 0
    ...
    if Model_Scope.Rows.Count:
        for hrow in Model_Scope.Rows:
            if hrow['Service_Product'] in SP_Discount.Keys:
                ...
                hrow['PY_SellPrice'] = str(float(hrow['Previous_Year_List_Price']) - (float(hrow['Previous_Year_List_Price']) * discount))
                ...
                hrow['PY_SellPrice'] = "0"
ScriptExecutor.Execute('PS_BGP_Scope Summary')