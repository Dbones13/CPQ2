...
if SC_Product_Type == 'New':
	...
	Model_Scope = Product.GetContainerByName("SC_Cyber_Models_Scope_Cont")
	CYB_Models_Hid_Cont = Product.GetContainerByName('SC_Cyber_Models_Cont_Hidden')
	CYB_Models_Hid_Cont.Clear()
	SP=[]
	summary=Product.GetContainerByName('SC_Cyber_Models_Cont')
	summary.Clear()
	# Fetching all values
...
if SC_Product_Type == 'Renewal':
	modelScopeCont = Product.GetContainerByName('SC_Cyber_Models_Scope_Cont')
	scopeCont = Product.GetContainerByName('SC_Cyber_Models_Cont')
	scopeContHidden = Product.GetContainerByName('SC_Cyber_Models_Cont_Hidden')
	...
	if modelScopeCont.Rows.Count:
		for row in modelScopeCont.Rows:
			...
			if int(row['Renewal_Quantity']) > int(row['PY_Quantity']) if row['PY_Quantity'] else 0:
				...
				scopeContRowHidden['SA_Quantity'] = str(int(row['Renewal_Quantity'])-int(row['PY_Quantity']) if row['PY_Quantity'] else 0)
				scopeContRowHidden['Comments'] = "Scope Addition"
			elif int(row['Renewal_Quantity']) < int(row['PY_Quantity']):
				scopeContRowHidden['SR_Quantity'] = str(int(row['Renewal_Quantity'])-int(row['PY_Quantity']) if row['PY_Quantity'] else 0)
			...
			if row['PY_Quantity'] not in ['','0']:
				scopeContRowHidden['SR_Price'] = str((float(row['PY_ListPrice'])/int(row['PY_Quantity']))*float(scopeContRowHidden['SR_Quantity']))
			scopeContRowHidden['SA_Price'] = str((float(row['HW_UnitPrice']) if row['HW_UnitPrice'] != '' else  0)*float(scopeContRowHidden['SA_Quantity']))
			if Product.Attr('SC_Pricing_Escalation').GetValue() == "Yes":
				if int(row['Renewal_Quantity']) > int(row['PY_Quantity']):
					scopeContRowHidden['List_Price'] = str(float(row['PY_ListPrice']) + ((float(row['Renewal_Quantity']) - float(row['PY_Quantity']))*float(row['HW_UnitPrice'])))
					scopeContRowHidden['Escalation_Price'] = row['PY_ListPrice']
				else:
					scopeContRowHidden['List_Price'] = str(float(row['Renewal_Quantity'])*(float(row['PY_ListPrice'])/float(row['PY_Quantity']) if row['PY_Quantity'] != '' else '0'))
					scopeContRowHidden['Escalation_Price'] = str(float(row['Renewal_Quantity'])*(float(row['PY_ListPrice'])/float(row['PY_Quantity']) if row['PY_Quantity'] != '' else '0'))
	...
	x = Product.Attr("SC_Cyber_RC_Selection").SelectedValues
