...
SPList = []
opt_ent_cont = Product.GetContainerByName('SC_Cyber_Optional_Ent_Cont')
serviceProduct = ""
...
if Product.Attr('SC_Product_Type').GetValue() == "Renewal":
    ...
    for record in resp["records"]:
        ...
        entitlement_query = SqlHelper.GetFirst("Select ServiceProduct,Entitlement,IsMandatory from CT_SC_ENTITLEMENTS_DATA where Product_Type = 'Cyber' and ServiceProduct = '{}'".format(product_name))
        if entitlement_query is not None:
            SPList.append(product_name)
            serviceProduct = product_name
            Product.Attr('SC_Cyber_Service_Product').SelectValues(*SPList)
