        ...
        for crow in CSContainer.Rows:
            ...
            if crow.IsSelected: #scope removal selected
                ...
                if crow['PY_Service_Prod_Status']=='Active': #active previous year service product is configured
                    ret_v=gsup.CheckServiceProductInContainer(Quote, Product, 'SC_Trace_Summary','Service_Product',crow['Service_Product'])
                    ...
                    if crow['CY_Service_Product'] not in (None,''):
                        if crow['CY_Service_Product'] in chk_dupl_list: 
                            ...
                            ret_v=gsup.CheckServiceProductInContainer(Quote, Product, 'SC_Trace_Summary','Service_Product',crow['CY_Service_Product']) #quote, product, Container name, column name, CY SP value
