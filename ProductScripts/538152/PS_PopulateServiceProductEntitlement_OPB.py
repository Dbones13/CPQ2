...
if active_contract and Product.Attr('SC_Product_Type').GetValue() == "Renewal":
    ...
    for record in resp["records"]:
        ...
        entitlement_query = SqlHelper.GetFirst("Select ServiceProduct from CT_SC_ENTITLEMENTS_DATA where Module_Name = 'Workforce Excellence Program' and ServiceProduct = '{}'".format(product_name))
        if entitlement_query is not None:
            sp_Ent.setdefault(product_name,[]).append(entitlement_name)

    if sp_Ent is not None:
        for key in sp_Ent.Keys:
            for value in sp_Ent[key]:
                row = sp_ent_cont.AddNewRow(False)
                row["Service_Product"] = key
