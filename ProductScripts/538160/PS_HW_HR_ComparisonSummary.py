...
if 'Scope Summary' in tabs:
	...
	if SC_Product_Type == 'Renewal':
		HW_Cont = Product.GetContainerByName('SC_ModelsSS_HR_RWL')
		if HW_Cont.Rows.Count:
			for row in HW_Cont.Rows:
				if Product.Name in ['Hardware Warranty','Hardware Refresh']:
					PreviousYearListPrice += float(row['PreviousYearListPrice'])
		...
		if ComparisonSummary.Rows.Count:
			for row in ComparisonSummary.Rows:
				if row['Service_Product']: # in ['Hardware Warranty','Hardware Refresh']:
					if value == "1":
