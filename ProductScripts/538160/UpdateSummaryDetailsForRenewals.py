...
if Product.Attr('SC_Product_Type').GetValue() is not None and Product.Attr('SC_Product_Type').GetValue() == 'Renewal':
	...
	if Product.Tabs.GetByName('Scope Summary').IsSelected == True:
		setDefaultValues = SqlHelper.GetList("select ServiceProduct, Entitlement from CT_SC_ENTITLEMENTS_DATA where IsMandatory = 'True' and ServiceProduct = '{}'".format(Product.Name))
		ENT_SS_CONT = Product.GetContainerByName('SC_ServicePrdEntSS_HR_RWL')
		ENT_SSE_CONT = Product.GetContainerByName('SC_Entitlements_HR_RWL')
		ENT_SS_CONT.Clear()
		if setDefaultValues is not None:
			for SP_Row in setDefaultValues:
				defRow = ENT_SS_CONT.AddNewRow(False)
				defRow["ServiceProduct"] = Product.Name
				defRow["Entitlement"] = SP_Row.Entitlement
				defRow["Type"] = "Mandatory"
		for row in ENT_SSE_CONT.Rows:
			if row.IsSelected:
				selRow = ENT_SS_CONT.AddNewRow(False)
				selRow["ServiceProduct"] = Product.Name
				selRow["Entitlement"] = row["SC_Entitlement_HR_RWL"]
				selRow["Type"] = "Optional"
		ENT_SS_CONT.Calculate()
		...
		for row in MODELS_SSE_CONT.Rows:
            ...
            #query = SqlHelper.GetFirst("Select Product,QuoteDetails from CT_SC_RENEWAL_QUOTE_TABLE where QuoteID = '{}' and Product = '{}'".format(reference_number,Product.Name))
			LastYearDiscountPer = str(round(float(py_discount1) * 100,2))
			#LastYearDiscountPer = float(eval(query.QuoteDetails[-1]['TotalDiscount'])) if query else 0
			...
			Trace.Write("PreviousYearSellPrice: "+str(PreviousYearSellPrice))
...
"+"PriceImpact --------->>		 		"+str(PriceImpact))
        ...
        if HRHW_Cont.Rows.Count:
			...
			for hrow,ss in zip(HRHW_Cont.Rows,sscont.Rows):
				if hrow:
					ss['PY_SellPrice'] = hrow['PY_SellPrice']
					Trace.Write("Lahu >>>>>>>>> " +str(ss['PY_SellPrice']))
        #sscont.Calculate()