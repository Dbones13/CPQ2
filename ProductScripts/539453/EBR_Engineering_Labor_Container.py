...
if scope == 'HW/SW + LABOR' and EBRvalue=='Yes':
    ...
    tableLabor = SqlHelper.GetList('select Top 1000 Deliverable,Productivity,GES_Eng_1_No,GES_Eng,FO_Eng_1_NoGES,FO_Eng_1_GES_None,FO_Eng_1_GES,FO_Eng_2_NoGES,FO_Eng_2_GES_None,FO_Eng_2_GES,Rank,Calculated_Hrs,Execution_Country from EBR_ENGINEERING_LABOR_CONTAINER Order By Rank')
    ...
    for row in tableLabor:
        if row.Deliverable not in disallow_lst and row.Deliverable not in current_deliverables:
            ...
            if gesLocation == "None" or gesLocation == '':
                ...
                if row.Deliverable in ['EBR Procure Materials & Services']:
                    new_row['GES Eng'] = ''
                else:
