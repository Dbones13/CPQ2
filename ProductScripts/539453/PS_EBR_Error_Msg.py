...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR' and  EBRvalue=='Yes' ):

    #Error Message for EBR_Engineering_Labor_Container
    deliverables = Product.GetContainerByName('EBR_Engineering_Labor_Container').Rows
    ...
    for row in deliverables:
        ...
        FO1_Eng = float(row.GetColumnByName('FO Eng 1 % Split').Value) if row.GetColumnByName('FO Eng 1 % Split').Value else 0

        FO_Eng_2_val = row.GetColumnByName('FO Eng 2').DisplayValue
        FO2_Eng = float(row.GetColumnByName('FO Eng 2 % Split').Value) if row.GetColumnByName('FO Eng 2 % Split').Value else 0
