...
Product.Attr('ErrorMessage3').AssignValue('')
HMI = Product.Attr('Is HMI Engineering in Scope?').GetValue()
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR' and HMI == 'Yes'):

    #Error Message for HMI_Engineering_Labor_Container
    deliverables = Product.GetContainerByName('HMI_Engineering_Labor_Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in HMI Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3
    if Labor_Message != '':
        Product.Attr('ErrorMessage3').AssignValue(Labor_Message)
