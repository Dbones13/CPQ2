...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message System_Interface_Engineering_Labor_Container
    deliverables = Product.GetContainerByName('System_Interface_Engineering_Labor_Container').Rows
    ...
    for row in deliverables:
        ...
        if (finalHrs not in ('','0','0.0') and (((FO_Eng_2_val != '' or FO_Eng_2_val != 'None') and FO2_Eng_Cost == 0 and FO2_Eng > 0) or ((FO_Eng_1_val != '' or FO_Eng_1_val != 'None') and FO1_Eng_Cost == 0 and FO1_Eng > 0) or ((GES_Eng_Val != '' or GES_Eng_Val != 'None') and GES_Eng_Cost == 0 and GES_Eng > 0))):
            count2 = count2 + 1
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in System Interface Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
