...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    deliverables = Product.GetContainerByName('System_Network_Engineering_Labor_Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in System Network Engineering (Leve 1 & 2). Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3
    if Labor_Message != ''and System_Network == 'Yes' :
