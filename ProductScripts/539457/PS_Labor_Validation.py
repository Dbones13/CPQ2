if Product.Name == "Virtualization System":
    ...
    if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
        ...
        #Error Message for SCADA_Engineering_Labor_Container
        deliverables = Product.GetContainerByName('Virtualization_Labor_Deliverable').Rows
        ...
        for row in deliverables:
            ...
            GES_Eng = FO1_Eng = FO2_Eng = FO1_Eng_Cost = FO2_Eng_Cost = 0
            ...
            if FO1_Eng_New < 0 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0'):
                count +=1
                row.GetColumnByName('Validation_Eng_Split').Value = '1'
            ...
            if ((FO_Eng_2_val == '' or FO_Eng_2_val == 'None') and FO2_Eng > 0 and FO2_Eng <= 100) or ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
                count1 = count1 + 1
                #row.GetColumnByName('Eng_Selection_Check').Value = '1'
                Labor_Message2 = Labor_Message2 + " - " +str(deliverable.Value) + "<br>"
            finalHrs = row.GetColumnByName('Final Hrs').Value
            if (finalHrs not in ('','0','0.0') and ((FO_Eng_2_val != '' and FO_Eng_2_val != 'None' and FO2_Eng_Cost == 0 and FO2_Eng > 0) or (FO_Eng_1_val != '' and FO_Eng_1_val != 'None' and FO1_Eng_Cost == 0 and FO1_Eng > 0))):
                count2 = count2 + 1
                Labor_Message3 = Labor_Message3 + " - " +str(deliverable.Value) + "<br>"
        laborAddi = Product.GetContainerByName('Virtualization_Additional_Custom_Deliverables')

        for row1 in laborAddi.Rows:
            eng_selec_chk = '0'
            val_eng_split = '0'
            FO_Eng_val = row1['FO Eng']
            FO_Eng = float(row1['FO Eng % Split']) if row1['FO Eng % Split'] not in ('','0') else 0.00
            GE_Eng_val = row1['GES Eng']
            GE_Eng = float(row1['GES Eng % Split']) if row1['GES Eng % Split'] not in ('','0') else 0.00
            if ((FO_Eng_val == '' or FO_Eng_val == 'None') and FO_Eng > 0) or ((GE_Eng_val == '' or GE_Eng_val == 'None') and GE_Eng > 0):
                eng_selec_chk = '1'
            row1['Eng_Selection_Check'] = eng_selec_chk
            row1.Calculate()
        laborAddi.Calculate()
        ...
        if count2 > 0:
            Labor_Message3 = "<b>"+'Cost is not available for selected resource in Experion Backup Restore Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
            if Labor_Message != "":
