...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for CE UOC Engineering Labor Container
    deliverables = Product.GetContainerByName('CE UOC Engineering Labor Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in CE UOC Engineering Labor Container. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3
    #Error Message for CE UOC Additional Custom Deliverables
    UOC_Error_Message_val = UOC_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('CE UOC Additional Custom Deliverables')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        """if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            UOC_Error_Message_val = UOC_Error_Message_val + " - " +str(deliverable) + "<br>" """
        #CXCPQ-24662
        ...
        if (finalHrs not in ('','0','0.0') and ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0 and deliverable != '')):
            count1 = count1 + 1
            UOC_Error_Message_val2 = UOC_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    """if count > 0:
        UOC_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + UOC_Error_Message_val
        if Labor_Message != "":
            Labor_Message += "<br>" + UOC_Error_Message_val
        else:
            Labor_Message = UOC_Error_Message_val """

    if count1 > 0:
        UOC_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + UOC_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + UOC_Error_Message_val2
        else:
            Labor_Message = UOC_Error_Message_val2
    if count2 > 0:
