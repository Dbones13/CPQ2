...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for PCD Engineering Labor Container
    deliverables = Product.GetContainerByName('PCD Engineering Labor Container').Rows
    gesLocation = gesLocation = Product.Attr("PCD_Ges_Location_Labour").GetValue()
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in ControlEdge PCD Engineering Labor Container. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for PCD Additional Custom Deliverables
    PCD_Error_Message_val = PCD_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('PCD Additional Custom Deliverables')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        if (finalHrs not in ('','0','0.0') and (FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
            count1 = count1 + 1
            PCD_Error_Message_val2 = PCD_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    if count1 > 0:
        PCD_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + PCD_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + PCD_Error_Message_val2
        else:
            Labor_Message = PCD_Error_Message_val2
    if count2 > 0:
