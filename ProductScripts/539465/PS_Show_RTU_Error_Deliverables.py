#PS_Show_RTU_Error_Deliverables
...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for CE RTU Engineering Labor Container
    deliverables = Product.GetContainerByName('CE RTU Engineering Labor Container').Rows
    ...
    for row in deliverables:
        ...
        exculdedList = ['RTU FEL Site Visit', 'RTU Procure Materials & Services', 'RTU Customer Training', 'RTU Project Close Out Report']
        if deliverable in exculdedList:
            GES_Eng = 0
        FO1_Eng_New = 0
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in CE RTU Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for CE RTU Additional Custom Deliverables
    RTU_Error_Message_val = RTU_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('CE RTU Additional Custom Deliverables')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        """
        #commented the if statement as it is not required
        if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0')  and deliverable != '':
            count +=1
            RTU_Error_Message_val = RTU_Error_Message_val + " - " +str(deliverable) + "<br>" """
        ...
        if ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0 and deliverable != ''):
            count1 = count1 + 1
            RTU_Error_Message_val2 = RTU_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    #commented the if statement as it is not required
    if count > 0:
        RTU_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + RTU_Error_Message_val
        if Labor_Message != "":
            Labor_Message += "<br>" + RTU_Error_Message_val
        else:
            Labor_Message = RTU_Error_Message_val"""

    if count1 > 0:
        RTU_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + RTU_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + RTU_Error_Message_val2
        else:
            Labor_Message = RTU_Error_Message_val2
    if count2 > 0:
