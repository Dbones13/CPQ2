...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for eServer_Labor_Additional_Cust_Deliverables_con
    eserver_Error_Message_val = eserver_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('eServer_Labor_Additional_Cust_Deliverables_con')
    count = count1 = 0
    for row in deliverables.Rows:
        ...
        if split != 100.0 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            eserver_Error_Message_val = eserver_Error_Message_val + " - " +str(deliverable) + "<br>"
        ...
        if (finalHrs not in ('','0','0.0') and ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0)):
            count1 = count1 + 1
            eserver_Error_Message_val2 = eserver_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    #if count > 0:
        #eserver_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + eserver_Error_Message_val
        #if Labor_Message != "":
            #Labor_Message += "<br>" + eserver_Error_Message_val
        #else:
            #Labor_Message = eserver_Error_Message_val

    if count1 > 0:
        eserver_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + eserver_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + eserver_Error_Message_val2
        else:
            Labor_Message = eserver_Error_Message_val2
    if count2 > 0:
