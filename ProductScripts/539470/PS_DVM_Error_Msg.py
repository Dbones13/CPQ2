...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for ARO_RESS_Engineering_Labor_Container Labor Container
    deliverables = Product.GetContainerByName('DVM_Engineering_Labor_Container').Rows
    ...
    for row in deliverables:
        ...
        if(GES_Eng in range(101) and FO2_Eng in range(101)):
            ...
            #Trace.Write("FO1_Eng_New"+str(FO1_Eng_New))
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in Digital Video Manager Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for ARO_RESS_Additional_Labour_Container
    Safety_Manager_Error_Message_val = Safety_Manager_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('DVM_Additional_Labour_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
