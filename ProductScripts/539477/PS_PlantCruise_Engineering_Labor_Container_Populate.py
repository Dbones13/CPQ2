...
if scope == 'HW/SW + LABOR':
    ...
    if Product.Attr("Labor_Site_Activities").GetValue() != 'Yes':
        disallow_lst.append("PlantCruise Site Installation")
        disallow_lst.append("PlantCruise Site Acceptance Test & Sign off")

    if Product.Attr("Labor_Operation_Manual_Scope").GetValue() != 'Yes':
        disallow_lst.append("PlantCruise Operation Manual")

    if Product.Attr("Labor_Custom_Scope").GetValue() != 'Yes':
        disallow_lst.append("PlantCruise Customer Training")

    if Product.Attr("CE_Cutover").GetValue() != 'Yes':
        disallow_lst.append("PlantCruise Cut Over Procedure")
    ...
    laborCont = Product.GetContainerByName('PlantCruise Engineering Labor Container')
    tableLabor = SqlHelper.GetList('select Deliverable,GES_Eng_NoGES,GES_Eng_GES,FO_Eng_1_NoGES,FO_Eng_1_GES,FO1_Eng_NoGES,FO1_Eng_GES,FO_Eng_2_NoGES,FO_Eng_2_GES,FO2_Eng_NoGES,FO2_Eng_GES,Rank,Execution_Country from PLANTCRUISE_ENGINEERING_DELIVERABLES')
    ...
    laborAddi = Product.GetContainerByName('PlantCruise Additional Custom Deliverables').Rows
