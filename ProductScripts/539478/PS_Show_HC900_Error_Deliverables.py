...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for HC900 Engineering Labor Container
    deliverables = Product.GetContainerByName('HC900 Engineering Labor Container').Rows
    gesLocation = gesLocation = Product.Attr("HC900_Ges_Location_Labour").GetValue()
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in HC900 Engineering Labor Container. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for HC900 Additional Custom Deliverables
    HC900_Error_Message_val = HC900_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('HC900 Additional Custom Deliverables')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        if (finalHrs not in ('','0','0.0') and (FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
            count1 = count1 + 1
            HC900_Error_Message_val2 = HC900_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    if count1 > 0:
        HC900_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + HC900_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + HC900_Error_Message_val2
        else:
            Labor_Message = HC900_Error_Message_val2
    if count2 > 0:
