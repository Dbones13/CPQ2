...
scope = 'HW/SW + LABOR' # Product.Attr('CE_Scope_Choices').GetValue()
Product.Attr('ErrorMessage').AssignValue('')
isProductLoaded = Product.Attr('isProductLoaded').GetValue()
if ('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR' and isProductLoaded == 'True':

    #Error Message for C300 Engineering Labor Container
    deliverables = Product.GetContainerByName('C300_Engineering_Labor_Container').Rows
    ...
    for row in deliverables:
        ...
        if(GES_Eng in range(101) and FO2_Eng in range(101)):
            FO1_Eng_New = getFloat(100 - GES_Eng - FO2_Eng)
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in C300 Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3
    #Error Message for C300 Additional Custom Deliverables
    additional_deliverables_error = additional_deliverables_error2 = ""
    deliverables = Product.GetContainerByName('C300_Additional_Custom_Deliverables_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        if(GES_Eng in range(101)):
            ...
            FO1_Eng_New = getFloat(100 - GES_Eng)
        if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            additional_deliverables_error = additional_deliverables_error + " - " +str(deliverable) + "<br>"
        ...
        if (finalHrs not in ('','0','0.0') and ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0)):
            count1 = count1 + 1
            additional_deliverables_error2 = additional_deliverables_error2 + " - " +str(deliverable) + "<br>"
    ...
    '''if count > 0:
        additional_deliverables_error =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + additional_deliverables_error
        if Labor_Message != "":
            Labor_Message += "<br>" + additional_deliverables_error
        else:
            Labor_Message = additional_deliverables_error'''

    if count1 > 0:
        additional_deliverables_error2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + additional_deliverables_error2
        if Labor_Message != "":
            Labor_Message += "<br>" + additional_deliverables_error2
        else:
            Labor_Message = additional_deliverables_error2
    if count2 > 0:
