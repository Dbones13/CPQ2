...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for PRMS_Engineering_Labor_Container Labor Container
    deliverables = Product.GetContainerByName('PRMS_Engineering_Labor_Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in PRMS Skid Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
    ...
    PRMS_Error_Message_val = PRMS_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('PRMS_Additional_Labor_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            PRMS_Error_Message_val = PRMS_Error_Message_val + " - " +str(deliverable) + "<br>"
        ...
        if (finalHrs not in ('','0','0.0') and (FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
            count1 = count1 + 1
            ColumnName = "Fo Eng"
            PRMS_Error_Message_val2 = PRMS_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    if count1 > 0:
        PRMS_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the {}% Split to 0% <br>'.format(ColumnName)+"</b>" + PRMS_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + PRMS_Error_Message_val2
        else:
            Labor_Message = PRMS_Error_Message_val2
    if count2 > 0:
