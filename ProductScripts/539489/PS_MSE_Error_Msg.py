...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for MSC_Engineering_Labor_Container Labor Container
    deliverables = Product.GetContainerByName('MSC_Engineering_Labor_Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in MeterSuite Engineering - MSC Functions. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for MSC_Additional_Labor_Container
    Safety_Manager_Error_Message_val = Safety_Manager_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('MSC_Additional_Labour_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
