...
if scope == 'HW/SW + LABOR':
    ...
    if process_type != "Yes":
        disallow_lst.append("Experion MX Site Installation")
        disallow_lst.append("Experion MX Site Acceptance Test & Sign off")
    process_type1 = Product.Attr('Labor_Operation_Manual_Scope').GetValue()
    if process_type1 != "Yes":
        disallow_lst.append("Experion MX Operation Manual")
    process_type2 = Product.Attr('Labor_Custom_Scope').GetValue()
    if process_type2 != "Yes":
        disallow_lst.append("Experion MX Customer Training")
    process_type3 = Product.Attr('CE_Cutover').GetValue()
    if process_type3 != "Yes":
        disallow_lst.append("Experion MX Cut Over Procedure")
    ...
    laborCont = Product.GetContainerByName('Experion_mx_Labor_Container')
    tableLabor = SqlHelper.GetList('select Deliverable,Productivity,GES_Eng_1_No,GES_Eng,FO_Eng_1_NoGES,FO_Eng_1_NEGES,FO_Eng_1_GES_None,FO_Eng_1_GES,FO_Eng_2_NoGES,FO_Eng_2_NEGES,FO_Eng_2_GES_None,FO_Eng_2_GES,Rank,Calculated_Hrs,Execution_Country from EXPERION_MX_LABOR_DELIVERABLES')
    ...
    laborAddi = Product.GetContainerByName('Experion_mx_labor_Additional_Cust_Deliverables_con').Rows
