Log.Info('IN PS_PLC_LI_Part_Summary')
def getFloat(Var):
    if Var:
        return float(Var)
    return 0

plc_cont_rows = Product.GetContainerByName('PLC_PartSummary_Cont').Rows
...
if errorFlag:
    ...
    #Product.Messages.Clear()
    plc_cont_rows = Product.GetContainerByName('PLC_PartSummary_Cont').Rows
    plc_li_cont = Product.GetContainerByName('PLC_PartSummary_LI_Cont')
