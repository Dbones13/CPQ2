...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for FDA_Engineering_Labor_Container Labor Container
    deliverables = Product.GetContainerByName('FDA_Engineering_Labor_Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in Fire Detection & Alarm Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for FDA_Additional_Labor_Container
    FDA_Error_Message_val = FDA_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('FDA_Additional_Labor_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            FDA_Error_Message_val = FDA_Error_Message_val + " - " +str(deliverable) + "<br>"
        ...
        if (finalHrs not in ('','0','0.0') and (FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
            count1 = count1 + 1
            FDA_Error_Message_val2 = FDA_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    """
    if count > 0:
        FDA_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + FDA_Error_Message_val
        if Labor_Message != "":
            Labor_Message += "<br>" + FDA_Error_Message_val
        else:
            Labor_Message = FDA_Error_Message_val"""

    if count1 > 0:
        FDA_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + FDA_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + FDA_Error_Message_val2
        else:
            Labor_Message = FDA_Error_Message_val2
    if count2 > 0:
