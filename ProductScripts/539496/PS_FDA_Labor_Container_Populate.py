...
disallow_lst = []
FDA_attr_value = Product.Attr('FDA IP scheme Required').GetValue()
if FDA_attr_value == "No":
    disallow_lst.append("FDA IP Addressing Scheme")
FDA_attr_value = Product.Attr('FDA Functional Description Required').GetValue()
if FDA_attr_value == "No":
    disallow_lst.append("FDA Functional Design Specification")
FDA_attr_value = Product.Attr('FDA XLS Software Configuration Document Required').GetValue()
if FDA_attr_value == "No":
    disallow_lst.append("FDA XLS Software Configuration Document")
FDA_attr_value = Product.Attr('FDA EBI Software Configuration Document Required').GetValue()
if FDA_attr_value == "No":
    disallow_lst.append("FDA EBI Software Configuration Document")
FDA_attr_value = Product.Attr('FDA Panels Networked').GetValue()
if FDA_attr_value == "No":
    disallow_lst.append("FDA XLS Network Configuration")
FDA_attr_value = Product.Attr('FDA Graphics Required').GetValue()
if FDA_attr_value == "No":
    disallow_lst.append("FDA Graphics Configuration")
FDA_attr_value = Product.Attr('FDA Interface Required').GetValue()
if FDA_attr_value == "No":
    disallow_lst.append("FDA Interface Configuration")
...
tableLabor = SqlHelper.GetList('select Deliverable,Productivity,GES_Eng_1,GES_Eng_1_None,GES_Eng,FO_Eng_1_NoGES,FO_Eng_1_GES_None,FO_Eng_1_GES,FO_Eng_2_NoGES,FO_Eng_2_GES_None,FO_Eng_2_GES,Rank,Calculated_Hrs,Execution_Country from FIRE_DETECTION_AND_ALARM_ENGINEERING_LABOR_CUSTOM_TABLE')
...
for row in tableLabor:
	if row.Deliverable not in disallow_lst and row.Deliverable not in current_deliverables:
		...
		if gesLocation != "None" and row.Deliverable in ["FDA Factory Acceptance Test", "FDA Site Acceptance Test", "FDA Commissioning & Startup Support"]:
			new_row.GetColumnByName('FO Eng 1').SetAttributeValue("SYS SSE-Eng")
			new_row['FO Eng 1']="SYS SSE-Eng"
		elif row.FO_Eng_1_NoGES:
			...
			if row.Deliverable == cont_row.GetColumnByName('Deliverable').Value:
				...
				if cont_row['GES Location'] != gesLocationVC:
					...
					if gesLocation == "None" or gesLocation == '':
						...
						if row.Deliverable in ["FDA Factory Acceptance Test", "FDA Site Acceptance Test", "FDA Commissioning & Startup Support"]:
							cont_row.GetColumnByName('FO Eng 1').SetAttributeValue(row.FO_Eng_1_NoGES)
					else:
						cont_row.GetColumnByName('GES Eng').ReferencingAttribute.SelectDisplayValue('SYS GES Eng-BO-'+gesLocationVC)
						if updateEngSplit:
							...
							if row.Deliverable in ["FDA Factory Acceptance Test", "FDA Site Acceptance Test", "FDA Commissioning & Startup Support"]:
								cont_row.GetColumnByName('FO Eng 1').SetAttributeValue("SYS SSE-Eng")
								cont_row['FO Eng 1']="SYS SSE-Eng"
					applyChanges = 1
				if applyChanges:
...
laborAddi = Product.GetContainerByName('FDA_Additional_Labor_Container').Rows
...
for row1 in laborAddi:
	...
	row1.Calculate()
Product.GetContainerByName('FDA_Additional_Labor_Container').Calculate()
