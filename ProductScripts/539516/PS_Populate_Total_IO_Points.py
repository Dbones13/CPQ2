...
for attr, conNames in conAttrDict.items():
    ...
    for conName in conNames:
        ...
        for row in con.Rows:
            for col in filter(lambda col:col.Name != "Rank" and not col.Name.endswith("Type") and not col.Name.startswith("Total"), row.Columns):
    ...
    rgTotal = sum(getInt(row[attr]) for row in Product.GetContainerByName('SM_RemoteGroup_Cont').Rows) if Product.GetContainerByName('SM_RemoteGroup_Cont') else 0
...
rgIsValue = sum(getInt(row['Labor_parameter_is']) for row in Product.GetContainerByName('SM_RemoteGroup_Cont').Rows) if Product.GetContainerByName('SM_RemoteGroup_Cont') else 0
