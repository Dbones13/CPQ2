...
#Remote Group
'''
PLC_remote_cont = Product.GetContainerByName('PLC_RemoteGroup_Cont')
for row in PLC_remote_cont.Rows:
    plc_remote_cont = row.Product.GetContainerByName('PLC_RG_PartSummary_Cont')
    plc_cab_remote_cont = row.Product.GetContainerByName('PLC_RG_Cabinet_Cont')
    ...
    #for val in plc_cab_remote_cont.Rows:
        #if  val['PLC_Cabinet_Type'] == 'One':
            #final_qty_R = int(round(qty1_R/2,0)+1)
        #elif val['PLC_Cabinet_Type'] == 'Dual':
            #final_qty_R = int(qty1_R)            
Product.Attr('PLC_Num_of_Sys_Cabinets_RG').AssignValue(str(qty1_R))
Product.Attr('PLC_Marshal_Cabinet_Qty_RG').AssignValue(str(Marshal_Qty_R))
'''