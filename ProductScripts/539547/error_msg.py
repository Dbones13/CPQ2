...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    deliverables = Product.GetContainerByName('SCADA_Engineering_Labor_Container').Rows
    ...
    for row in deliverables:
        ...
        if FO1_Eng_New < 0 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0'):
            ...
            row.GetColumnByName('Validation_Eng_Split').Value = '0'
        ...
        if (finalHrs not in ('','0','0.0') and (((FO_Eng_2_val == '' or FO_Eng_2_val == 'None') and FO2_Eng > 0 and FO2_Eng <= 100) or ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO_Eng_1_val > 0))):
            count1 = count1 + 1
    ...
    laborAddi = Product.GetContainerByName('SCADA_Additional_Custom_Deliverables_Container')

    for row1 in laborAddi.Rows:
        eng_selec_chk = '0'
        val_eng_split = '0'
        FO_Eng_val = row1['FO Eng']
        FO_Eng = float(row1['FO Eng % Split'])
        GE_Eng_val = row1['GES Eng']
        GE_Eng = float(row1['GES Eng % Split'])
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in Scada Engineering. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
