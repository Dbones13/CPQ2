...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for SCADA_Additional_Custom_Deliverables_Container
    scada_Error_Message_val = scada_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('SCADA_Additional_Custom_Deliverables_Container')
    count = count1 = 0
    for row in deliverables.Rows:
        deliverable = row["Deliverable Name"] if row["Deliverable Name"] else row['Standard Deliverable']
        GES_Eng = FO1_Eng = FO1_Eng_Cost = FO2_Eng_Cost = GES_Eng_Cost = 0
        ...
        if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            scada_Error_Message_val = scada_Error_Message_val + " - " +str(deliverable) + "<br>"
        ...
        if (finalHrs not in ('','0','0.0') and ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0)):
            count1 = count1 + 1
            scada_Error_Message_val2 = scada_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    if count > 0:
        scada_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + scada_Error_Message_val
        if Labor_Message != "":
            Labor_Message += "<br>" + scada_Error_Message_val
        else:
            Labor_Message = scada_Error_Message_val

    if count1 > 0:
        scada_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + scada_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + scada_Error_Message_val2
        else:
            Labor_Message = scada_Error_Message_val2
    if count2 > 0:
