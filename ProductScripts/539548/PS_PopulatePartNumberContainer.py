...
if not isR2Qquote:
    ...
    def checkForMPACustomer():
        ...
        query = TagParserQuote.ParseString("select * from MPA_PRICE_PLAN_MAPPING where Honeywell_Ref = '<*CTX(Quote.CustomField(MPA Honeywell Ref))*>' and Honeywell_Ref ! = '' and Honeywell_Ref != '' and Price_Plan_Status= 'Active' and Price_Plan_Parts_Discount = 'Y' and Price_Plan_Start_Date < '<*CTX( Date.Format(MM/dd/yyyy) )*>' and Price_Plan_End_Date > '<*CTX( Date.Format(MM/dd/yyyy) )*>'")
    ...
    def populateFOPartNumberCon(foPartNumberCon,mpaAvailable,activeServiceContract,executionYear,SelectedProducts):
        partsList = []
        ...
        queryData1 = SqlHelper.GetList("select fo.*,p.PRODUCT_NAME from Labor_FO_Part_Number fo join products p on p.PRODUCT_CATALOG_CODE = fo.GES_Part_Number where fo.GES_Part_Number != '' and fo.LOB = '"+LOB+"'and fo.Product_Module IN ("+SelectedProducts+") and fo.GES_Location = '{}'".format(TagParserProduct.ParseString('<*ValueCode(MSID_GES_Location)*>')))
    ...
    MSID_Selected_Products=Product.Attr("MSID_Selected_Products").GetValue()
    ...
    populateFOPartNumberCon(foPartNumberCon,mpaAvailable,activeServiceContract,executionYear,SelectedProducts)
    if Product.ParseString('[LIKE](<*VALUE(MSID_Selected_Products)*>,3rd Party PLC to ControlEdge PLC/UOC)'):
        Product.ApplyRules()