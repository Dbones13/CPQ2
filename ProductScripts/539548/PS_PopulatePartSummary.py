if Session["Product Loading"] != True:
	...
	if not isR2Qquote:
		...
		def migrationAttrDict(product):
			...
			updateStandAloneAttributesDict(product, migrationAttrDict)
			if product.Name == 'OPM':
				...
				containersNames = []
			updateAttributeDict(product, migrationAttrDict, containersNames)
			if product.Name == 'LCN One Time Upgrade':
				...
				contNames = []
			updateAttributeDictWithMultiplier(product, migrationAttrDict, contNames)
			if product.Name == 'Orion Console':
			...
			if Product.Attr('MIgration_Scope_Choices').GetValue():
				if product.Name == 'FSC to SM':
					...
					updateAttrDictWithCustomFDMUpgrade2(product, migrationAttrDict)
				elif product.Name == 'FDM Upgrade 3':
					updateAttrDictWithCustomFDMUpgrade3(product, migrationAttrDict)
				elif product.Name == 'Orion Console':
					updateAttributeDictWithCustomOrion(product, migrationAttrDict)
				elif product.Name == 'EHPM/EHPMX/ C300PM':
					updateAttrDictWithCustomXpm(product, migrationAttrDict, Quote, Product.Attr('SESP_TYPE').GetValue())
				elif product.Name == 'TPS to Experion':
				...
				elif product.Name == 'FSC to SM IO Migration':
					FSCtoSM_IOparts(product, migrationAttrDict, Product)
				elif product.Name == 'C200 Migration':
					...
					updateAttrDictWithCustomxPMC300(product, migrationAttrDict, Quote, Product)
				elif product.Name == 'XP10 Actuator Upgrade':
					updateAttrDictWithXP10Actuator(product, migrationAttrDict)
				if Product.Attr("MIgration_Scope_Choices").GetValue() not in ["LABOR"]:
					if product.Name == 'CWS RAE Upgrade':
					...
					elif product.Name == 'TPA/PMD Migration':
				...
				if product.Name == 'C200 Migration':
					...
					TypeofUOC = product.Attr('C200_Select_Migration_Scenario').GetValue()
		...
		def updateProductDict(dictToUpdate, row, qty):
			if row.IsThirdParty:
				...
				#Log.Info('row.Child_Part = ' + str(row.Child_Part) + ' Quantity = ' + str(partQty + qty))
		...
		def getPartsToBeAdded(attributeValueDict,mappingPart):
			...
			#Log.Write('----partNumbersToBeAdded--ps---' + str(partNumbersToBeAdded))
		...
		def updateStandAloneAttributesDict(product, attributeValueDict):
			for attr in product.Attributes:
				if attr.DisplayType not in ['Container', 'DisplayOnlyText', 'CheckBox']:
					mappingName = getProductAttrMappingName(product.Name, attr.Name)
		...
		containerNameMapping = {
			...
			"FDM_Upgrade": getContainer(Product, "MSID_FDM_Upgrade_1_Added_Parts_Common_Container"),
			"FDM_Upgrade_2": getContainer(Product, "MSID_FDM_Upgrade_2_Added_Parts_Common_Container"),
			"FDM_Upgrade_3": getContainer(Product, "MSID_FDM_Upgrade_3_Added_Parts_Common_Container"),
		...
		sesp_type=Product.Attr('SESP_TYPE').GetValue()
		Product.Attr('KE_SESP').AssignValue(sesp_type)
		partMapping_forSel={
			'OPM':'OPM',
		...
		for selectedRow in selectedProducts.Rows:
			...
			attributeValueDict = migrationAttrDict(selectedRow.Product)
			...
			if scope =="LABOR": # added logic to fix CXCPQ-109620
				laborpartlist=set()
				for i in partsList:
					if i[:3] in ['SVC']:
						partlist.add(i)
				partsList=laborpartlist
			...
			if selectedRow.Product.Name == '3rd Party PLC to ControlEdge PLC/UOC' and scope != 'LABOR':
				popthirdPartSummary(selectedRow.Product, partNumbersToBeAdded, partsList, Product)
			if scope != "HW/SW":
				getProductLaborDeliverablesContainer(selectedRow.Product.Name, partNumbersToBeAdded, partsList, Product)
			...
			if selectedRow.Product.Name == 'Virtualization System Migration' and selectedRow.Product.GetContainerByName("Virtualization_partsummary_cont"):
				for row in selectedRow.Product.GetContainerByName("Virtualization_partsummary_cont").Rows:
					partsList.add(row['partnumber'])
			...
			for product, parts in partNumbersToBeAdded.items():
				...
				if product == "THIRD_PARTY":
					...
					for part, qty in parts.items():
						...
						if qty > 0.00 and qty != '' and part not in ShipcrateParts and part in list(partsList):
							#Trace.Write("quantity for svc-- "+str(qty)+ "part--->" +str(part))
							row = None
							for existingRow in container.Rows:
							...
							if selectedRow.Product.Name == "ELCN" and (part=="MZ-PCSR02" or "MZ-PCSR02" in partDetailsDict):
								MZ_PCSR02_qty=qty
							if selectedRow.Product.Name == "TPS to Experion" and part in TPS_Sub_parts:
								Total_TPSSubparts_qty+=Total_TPSSubparts_qty
							Product.Attr('CratepartsQuantity').AssignValue(str(Total_TPSSubparts_qty))
			...
			R2QFlag = True if Quote.GetCustomField("R2QFlag").Content else False
			if R2QFlag:
				if selectedRow.Product.Name == 'Virtualization System Migration' and selectedRow.Product.GetContainerByName("Virtualization_partsummary_cont"):
						existingParts = set()
						for row in Product.GetContainerByName("MSID_Virtualization_Added_Parts_Common_Container").Rows:
							existingParts.add(row['PartNumber'])
						for Prd_Trace in selectedRow.Product.GetContainerByName("Virtualization_partsummary_cont").Rows:
							part = Prd_Trace['partnumber']
							final_qty = float(Prd_Trace['CE_Final_Quantity']) if Prd_Trace['CE_Final_Quantity'] else 0
							if final_qty > 0:
								if part not in existingParts:
									MSID_Trace = Product.GetContainerByName("MSID_Virtualization_Added_Parts_Common_Container").AddNewRow(False)
									MSID_Trace ['PartNumber'] = part
									MSID_Trace ['Quantity'] = Prd_Trace['CE_Part_Qty']
									MSID_Trace ['PartDescription'] = Prd_Trace['CE_Part_Description']
								if part in partDetailsDict:
									MSID_Trace ['PLSG'] = partDetailsDict[part][1]
									MSID_Trace ['plsgDescription'] = partDetailsDict[part][2]
