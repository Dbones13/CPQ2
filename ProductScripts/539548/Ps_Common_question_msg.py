...
if prdcont.Rows.Count > 0:
	...
	for row in prdcont.Rows:
			selectedprd.append(row['Selected_Products'])
			if row['Selected_Products'] == 'Project Management':
				selectedprd.remove(row['Selected_Products'])

selectedprd = set(list(filter(None, selectedprd)))
visible_prd = set([prduct for prduct in baseProd if prduct in selectedprd])

Trace.Write("selectedprd::" +str(selectedprd))
Trace.Write("visible_prd-> "+str(visible_prd))
if (selectedprd == visible_prd):
	Trace.Write("completed::")
	Product.Attr('Attr_Incomplete_Val').AssignValue('1')
else:
	Trace.Write("error::")
	Product.Attr('Attr_Incomplete_Val').AssignValue('0')
if Quote.GetCustomField("isR2QRequest").Content == 'Yes':
	...
	Product.Attr('MIgration_Scope_Choices').Access = AttributeAccess.ReadOnly
	Product.Attr('MSID_Active_Service_Contract').Access = AttributeAccess.ReadOnly
else:
