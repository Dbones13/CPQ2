...
def getcontainersData(container,product):
	...
	Trace.Write("deliverablelist"+str(adtnl_deliver))
	Trace.Write("containerdata"+str(containerData))
...
def populateCommonCon(product,container):
	...
	elif product in ("Virtualization System", "Virtualization System Migration"):
...
saver2q = Quote.GetCustomField('R2Q_save').Content
r2qrequest = Quote.GetCustomField('IsR2QRequest').Content
...
if msidContainer:
	for row in msidContainer.Rows:
		...
		msidproductCon = msidProduct.GetContainerByName("CONT_MSID_SUBPRD")
		msidNumber = msidProduct.Attr('Migration_MSID_Choices').GetValue()
		productCon = dict()
		msidproductVirtCon = msidProduct.GetContainerByName("CONT_MSID_SUBPRD")
		msidproductGenCon = msidProduct.GetContainerByName("MSID_Product_Container_Generic_hidden")
		if msidproductCon:
			for row in msidproductCon.Rows:
				productCon[row["Selected_Products"]] = row.UniqueIdentifier
		...
		traceSoftwareCon = getContainer(msidProduct,"Trace_Software_Labor_con")
		...
		if productCon.get("FDM Upgrade 1") or productCon.get("FDM Upgrade 2") or productCon.get("FDM Upgrade 3"):
		...
		if productCon.get("Virtualization System Migration"):
			getcontainersData(virtualizationCon,"Virtualization System Migration")
...
populateCommonCon("EHPM/EHPMX/ C300PM",getContainer1("Migration_Labor_EHPM_Common"))
...
populateCommonCon("Virtualization System Migration",getContainer1("Migration_Labor_Virtualization_System_Common"))
...
if saver2q == 'Submit' and r2qrequest:
    populateCommonCon("Trace Software",getContainer1("Trace_Software_Labor_con"))