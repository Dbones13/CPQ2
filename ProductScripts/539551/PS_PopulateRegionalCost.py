...
if saveAction != 'Save':
	...
	for row in msidContainer.Rows:
		...
		traceCont = getContainer(msidProduct,"Trace_Software_Labor_con")
		...
		if productCon.get("FDM Upgrade 1"):
			getpartNumbersData(fdmCon,"FDM Upgrade 1")
		if productCon.get("FDM Upgrade 2"):
			getpartNumbersData(fdmCon,"FDM Upgrade 2")
		if productCon.get("FDM Upgrade 3"):
			getpartNumbersData(fdmCon,"FDM Upgrade 3")
		...
		if productCon.get("Virtualization System Migration"):
			Trace.Write("VirtualizationSystem")
			getpartNumbersData(virtualizationCon,"Virtualization System Migration")
		#if productCon.get("Virtualization System Migration"):
			#Trace.Write("VirtualizationSystem")
	...
	#-- Janhavi Tanna : CXCPQ-60180 :end
		...
		if productCon.get("Trace Software"):
			Trace.Write("Trace Software")
			getpartNumbersData(traceCont,"Trace Software")
		Log.Info("containerdata"+str(partsNumbers))
		Trace.Write("containerrecord"+str(msidProduct.GetContainerByName("MSID_Labor_Orion_Console_Con").Rows.Count))
		#Trace.Write(str(laborParts))
		#Trace.Write(str(laborPartsListPrice))
		if partsNumbers:
			#Log.Info(" outside for ==> PartNum -=>" +str(item.PartNumber))
