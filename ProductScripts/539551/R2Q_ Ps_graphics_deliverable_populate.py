...
if isR2Qquote:
	...
	for MigrationNew in migration_new_cont.Rows:
		msid_scope=MigrationNew.Product.Attr('MIgration_Scope_Choices').GetValue()
		for frow in MigrationNew.Product.GetContainerByName("CONT_MSID_SUBPRD").Rows:
			selectedprd = frow.Product
			...
			if frow["Selected_Products"] == 'Graphics Migration' and msid_scope in ('HW/SW/LABOR','LABOR') :
				...
				containers = MigrationNew.Product.GetContainerByName("MSID_Labor_Graphics_Migration_con")
				if MigrationNew.Product.Attr("MSID_GES_Location").GetValue() != 'None':
			...
			if frow["Selected_Products"] == 'TPA/PMD Migration' and msid_scope in ('HW/SW/LABOR','LABOR') :
				container = MigrationNew.Product.GetContainerByName("MSID_Labor_TPA_con")
