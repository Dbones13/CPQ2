...
if isR2Qquote :
	from GS_MigrationLaborHoursModule_new import getnumberOfjumpRealease,getRowData,getRowDataIndex,checkForMPACustomer,calculateTotals,getContainer,getFloat
	from GS_MigrationLaborHoursModule_2_new import getxPMtoC300LabourHours,calculateFinalHours1,reCalAdj
	from GS_MigrationLaborHoursModule_3New import getFSCTOSMLabourHours,getFDMLabourHours,getXP10LabourHours,getLMtoELMMLabourHours,getFSCtoSMIOAuditLabourHours
	from GS_MigrationLaborHoursModule_4New import getGraphicsMigrationLabourHours,getCDActuatorLabourHours,getCWSRAEMigrationLabourHours,getFSCTOSMIOLabourHours,calculateEMPEfforts
	...
	for MigrationNew in migration_new_cont.Rows:
		...
		cwsCon = getContainer(MigrationNew.Product,"MSID_Labor_CWS_RAE_Upgrade_con")
		...
		allSelectedProducts = MigrationNew.Product.Attr("MSID_Selected_Products").GetValue()
		fdmLaborExecutionFlag = False
		for row in msidCont.Rows:
			Product = row.Product
			selectedProducts = row["Selected_Products"]
				
			if xPMCon.Rows.Count > 0 and 'xPM to C300 Migration' in selectedProducts :
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
				felSiteVisitDataGathering,migrationDDS,inHouseEngineering,fatProcedure,preFAT,fat,siteInstallationEaps,siteInstallationEST1,commisioning,sat = getxPMtoC300LabourHours(Product,msid_product)
			...
			if fsctosmCon.Rows.Count > 0 and fsctosmauditCon.Rows.Count > 0 and selectedProducts =='FSC to SM' :
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
				...
				for row in fsctosmCon.Rows:
					...
					if row["Deliverable"] == "Documentation":
						...
						row["Adjustment_Productivity"] = reCalAdj(row,oldCalHrs)
						row["Final_Hrs"] = calculateFinalHours1(row,oldCalHrs)
				calculateEMPEfforts(msid_product,fsctosmCon)    
				...
				for row in fsctosmauditCon.Rows:
					...
					if row["Deliverable"] == "Pre-Migration Audit":
						...
						row["Adjustment_Productivity"] = reCalAdj(row,oldCalHrs)
						row["Final_Hrs"] = calculateFinalHours1(row,oldCalHrs)
				calculateEMPEfforts(msid_product,fsctosmauditCon)        
				calculateTotals(fsctosmCon)
				calculateTotals(fsctosmauditCon)

			if fdmCon.Rows.Count > 0 and not fdmLaborExecutionFlag and ('FDM Upgrade 1' in allSelectedProducts or 'FDM Upgrade 2' in allSelectedProducts or 'FDM Upgrade 3' in allSelectedProducts)  :
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
				Planning_and_Kom, System_backup, Ordering_Software_Hardware, installation_and_config,Documentation_Workpack_FAT_Checklist_SAT_Checklist, interal_testing, Commissioning, SAT, AS_Built_Documentation_and_Corrective_Actions, Project_Closeout = getFDMLabourHours(msidCont,allSelectedProducts)
				fdmLaborExecutionFlag = True
			...
			if XP10Con.Rows.Count > 0 and 'XP10 Actuator Upgrade' in selectedProducts:
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
			...
			if lmCon.Rows.Count > 0 and 'LM to ELMM ControlEdge PLC' in selectedProducts:
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
				plan_esss, plan_eaps, feldatagathering, migrationdds_esss, migrationdds_eaps, sw_hworder, inhouse_esss, inhouse_eaps, prefat, fat, siteinstallation_eaps, siteinstallation_est, sat = getLMtoELMMLabourHours(Quote,Product,msid_product)
			...
			if GraphicsCon.Rows.Count > 0 and 'Graphics Migration' in selectedProducts:
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
				customer_input_Study, Query_Generation_Clarification, Display_Generation, Safeview_Configuration, Testing_system, SAT_Support, Operator_Training, Project_Close_Out, Shapes, Migration_FDS, FAT_Support, GAP_Analysis, FAT_SAT_Documentation, Migration_DDS, Faceplates = getGraphicsMigrationLabourHours(Product,msid_product)
			...
			if CDActuatorCon.Rows.Count > 0 and 'CD Actuator I-F Upgrade' in selectedProducts:
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
			...
			if fscsmioCon.Rows.Count > 0 and selectedProducts =='FSC to SM IO Migration' :
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
			...
			if cwsCon.Rows.Count > 0 and 'CWS RAE Upgrade' in selectedProducts:
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
			...
			if fsctosmioauditcon.Rows.Count > 0 and selectedProducts == 'FSC to SM IO Migration':
				#msidCont = Product.GetContainerByName("CONT_MSID_SUBPRD")
				#for row in msidCont.Rows:
					#Product = row.Product
        	...
        	msidContainer = MigrationNew.Product.GetContainerByName("MSID_Labor_xPM_to_C300_Migration_Con")
	    	for row in msidContainer.Rows:
		   		Trace.Write("XPM Labor Data"+row['Deliverable']+"Calculated_Hrs"+row['Calculated_Hrs']+"FO_Eng"+row['FO_Eng']+"ges_eng"+row['GES_Eng'])       
