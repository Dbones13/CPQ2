...
if isR2Qquote:
	...
	def getPartsCost(Product):
		partsCost = dict()
		foPartNumberCon = getContainer(Product, "MSID_Labor_FO_Part_Number")
	...
	def laborCostWithCOnversion(laborcostParts):
		quoteCurrency = Quote.GetCustomField("SC_CF_CURRENCY").Content if Quote.GetCustomField("R2QFlag").Content == 'Yes' else Quote.SelectedMarket.CurrencyCode
			...
			for key in laborcostParts:
				if quoteCurrency == "USD" or laborcostParts[key]["stdcurrency"] == "USD":
					...
					if query1 is not None:
						factor = factor * getFloat(query1.Exchange_Rate) # 0.011514104778353483
						...
						if queryUSD is not None:
							factor = factor * getFloat(queryUSD.Exchange_Rate) # 0.0181324122049510650284
						else:
							factor = 1.00
	...
	def CostValidation(Product, container_name):
		deliverables = ''
	...
	for MigrationNew in migration_new_cont.Rows:
		partsListPrice = getPartsListPrice(MigrationNew.Product)
		...
		partsCost = getPartsCost(MigrationNew.Product)
		...
		for container in containers:
			for row in getContainer(MigrationNew.Product, container).Rows:
				if row["Deliverable"] not in ('Total','Off-Site','On-Site'):
					populateGESListPrice(row)
					populateGESCost(row,defaultExecutionYear)
				getContainer(MigrationNew.Product, container).Calculate()
	...
	if not isR2Qquote:
		for container in containers:
			if container != "MSID_Additional_Custom_Deliverables":
				deliverables = CostValidation(MigrationNew.Product, container)
				if len(deliverables) > 0:
					message += deliverables
		if len(message) > 0:
			if MigrationNew.Product.Attr("MIgration_Scope_Choices").GetValue() != "HW/SW":
				MigrationNew.Product.Attr('ErrorMessage').AssignValue(message)
			else:
				MigrationNew.Product.Attr('ErrorMessage').AssignValue("")