...
if isR2Qquote:
	...
	def populatelVirtualizationLabCon(Product, LabCon,AdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,modulename):
			...
			for entry in queryData:
				if Product.ParseString(entry.Deliverable_Type) == "Offsite":
					...
					if GES_Loc:
						...
						if row["GES_Eng"] == '':
							row["GES_Eng"] = "SVC_GES_P350B_{}".format(Product.ParseString('<*ValueCode(MSID_GES_Location)*>'))
					else:
						if row["FO_Eng_Percentage_Split"] == '':
				...
				if Product.ParseString(entry.Deliverable_Type) == "Onsite":
					...
					if GES_Loc:
						...
						if row["GES_Eng_Percentage_Split"] == '':
							row["GES_Eng_Percentage_Split"] = Product.ParseString(entry.GES_Eng_Split)
						if row["GES_Eng"] == '':
							row["GES_Eng"] = "SVC_GES_P350F_{}".format(Product.ParseString('<*ValueCode(MSID_GES_Location)*>'))
					else:
						if row["FO_Eng_Percentage_Split"] == '':
							row["FO_Eng_Percentage_Split"] = "100"
						if row["GES_Eng_Percentage_Split"] == '':
	...
	def populatelGenericLabCon(Product, LabCon,AdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,modulename):
		#if LabCon.Rows.Count == 0:
		excecutionCountry = getExecutionCountry()
	...
	def populatelQCSRAELabCon(Product, LabCon,AdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,modulename):
		#if LabCon.Rows.Count == 0:
		excecutionCountry = getExecutionCountry()
	...
	def populatelTPALabCon(Product, LabCon,AdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,modulename,msid_product):
		#if LabCon.Rows.Count == 0:
		excecutionCountry = getExecutionCountry()
	...
	def populatelELEPIULabCon(Product, LabCon,AdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,modulename):
		#if LabCon.Rows.Count == 0:
		excecutionCountry = getExecutionCountry()
	...
	for MigrationNew in migration_new_cont.Rows:
		if MigrationNew.Product.Attr('MIgration_Scope_Choices').GetValue() != "HW/SW":
			...
			virtualizationAdjustmentProductivity = getAttrValue(MigrationNew.Product, "Virtualization_Adjustment_Productivity") if getAttrValue(MigrationNew.Product, "Virtualization_Adjustment_Productivity") else "1"
			QCSRAECon = getContainer(MigrationNew.Product, "MSID_Labor_QCS_RAE_Upgrade_con")
			QCSRAEAdjustmentProductivity = getAttrValue(MigrationNew.Product,"QCS_RAE_Upgrade_Adjustment_Productivity")
			TPACon = getContainer(MigrationNew.Product, "MSID_Labor_TPA_con")
			TPAAdjustmentProductivity = getAttrValue(MigrationNew.Product, "TPA_Adjustment_Productivity")
			# To have few common functionalities for Labor deliverables tab -- Janhavi Tanna : CXCPQ-60159 :start
			ELEPIUCon = getContainer(MigrationNew.Product, "MSID_Labor_ELEPIU_con")
			ELEPIUAdjustmentProductivity = getAttrValue(MigrationNew.Product, "ELEPIU_Adjustment_Productivity")
			#-- Janhavi Tanna : CXCPQ-60159 :end
			...
			GES_Loc=getAttrValue(MigrationNew.Product, "MSID_GES_Location") if getAttrValue(MigrationNew.Product, "MSID_GES_Location") != 'None' else ''
			GES_VC= MigrationNew.Product.Attr('MSID_GES_Location').SelectedValue.ValueCode if MigrationNew.Product.Attr('MSID_GES_Location').SelectedValue else ''
			msidnewproduct = MigrationNew.Product
			...
			for genrow in msidCont.Rows:
				if 'Generic System' in genrow['Product Name']:
					...
					if genrow['Product Name'] == 'Generic System 1':
						MigrationNew.Product.Attr('MSID_Labor_Generic_System1_Cont').LabelFormula = str(genrow['User_Define_Name']) if genrow['User_Define_Name'] else '<!-->'
					elif genrow['Product Name'] == 'Generic System 2' :
						MigrationNew.Product.Attr('MSID_Labor_Generic_System2_Cont').LabelFormula = str(genrow['User_Define_Name']) if genrow['User_Define_Name'] else '<!-->'
					elif genrow['Product Name'] == 'Generic System 3':
						MigrationNew.Product.Attr('MSID_Labor_Generic_System3_Cont').LabelFormula = str(genrow['User_Define_Name']) if genrow['User_Define_Name'] else '<!-->'
					elif genrow['Product Name'] == 'Generic System 4' :
						MigrationNew.Product.Attr('MSID_Labor_Generic_System4_Cont').LabelFormula = str(genrow['User_Define_Name']) if genrow['User_Define_Name'] else '<!-->'
					elif genrow['Product Name'] == 'Generic System 5' :
						MigrationNew.Product.Attr('MSID_Labor_Generic_System5_Cont').LabelFormula = str(genrow['User_Define_Name']) if genrow['User_Define_Name'] else '<!-->'
			...
			for row in msidCont.Rows:
				...
				populatelVirtualizationLabCon(MigrationNew.Product, virtualizationCon,virtualizationAdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,"Virtualization System") if "Virtualization System" in selectedProducts else 0
				populatelQCSRAELabCon( MigrationNew.Product, QCSRAECon,QCSRAEAdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,"QCS RAE Upgrade") if "QCS RAE Upgrade" in selectedProducts else 0
				populatelTPALabCon(MigrationNew.Product, TPACon,TPAAdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,"TPA/PMD Migration",msid_product) if "TPA/PMD Migration" in selectedProducts else 0
				populatelELEPIULabCon( MigrationNew.Product, ELEPIUCon,ELEPIUAdjustmentProductivity,mpaAvailable,activeServiceContract,contractType,"ELEPIU ControlEdge RTU Migration Engineering") if "ELEPIU ControlEdge RTU Migration Engineering" in selectedProducts else 0
				if 'Generic System Migration' in selectedProducts:
					...
					for i in range(1,gensyscount+1):
						...
						#cont.Clear()
						populatelGenericLabCon(MigrationNew.Product, cont,str(1),mpaAvailable,activeServiceContract,contractType,"Generic System")