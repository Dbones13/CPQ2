...
if isR2Qquote:
	from GS_MigrationLaborHoursModule_new import getnumberOfjumpRealease
	import math
			...
			if row['Selected_Products'] == 'OPM':
				...
				for key in parameters:
					if key == "MSID_CommonQuestions":
						Var_22 = getAttrData(msid_product,parameters[key]["Var_22"])
						Var_5 = getAttrData(msid_product,parameters[key]["Var_5"])
	...
	migration_new_cont = Product.GetContainerByName('CONT_Migration_MSID_Selection')
	for MigrationNew in migration_new_cont.Rows:
		selectedProducts = MigrationNew.Product.Attr('MSID_Selected_Products').GetValue().split('<br>')
		if 'OPM' in selectedProducts:
			...
			mcoe_hrs = 0
			salesOrg = ''
			opmEng = getContainer(MigrationNew.Product,'MSID_Labor_OPM_Engineering')
			...
			if eac_per > 0 and amt == 'Yes':
				MigrationNew.Product.AllowAttr('OPM_LaborAMT_Details')
			else:
				MigrationNew.Product.DisallowAttr("OPM_LaborAMT_Details")
				MigrationNew.Product.GetContainerByName('OPM_LaborAMT_Details').Rows.Clear()

			if 'OPM' in selectedProducts and scope != "HW/SW" and eac_per > 0:
				amt_cont = getContainer(MigrationNew.Product,'OPM_LaborAMT_Details')
