...
if isR2Qquote:
	...
	for MigrationNew in migration_new_cont.Rows:
		container1 = MigrationNew.Product.GetContainerByName('MSID_Labor_Virtualization_con')
		scope = MigrationNew.Product.Attr('MIgration_Scope_Choices').GetValue()
		for item in MigrationNew.Product.GetContainerByName('CONT_MSID_SUBPRD').Rows:
			if item['Selected_Products'] in ('Virtualization System Migration', 'Virtualization System'):
		...
		if (scope == 'HW/SW/LABOR' or scope == 'LABOR') and (Virtualization_hidden_cont >0):
			...
			RMP_Effort = MigrationNew.Product.Attr('Regional_Migration_Principal_Efforts_Required').GetValue()
			...
			for row in container1.Rows:
				...
				if RMP_Effort=="Yes" and row["Deliverable"] == "Regional Migration Principal Efforts":
					...
					MigrationNew.Product.Attr('msid_Virtualization_RMP_Efforts').AssignValue(str(Principle_eff_final_hrs))
					...
					row["Final_Hrs"]=calculateFinalHours1(row,oldCalHrs)
					MigrationNew.Product.Attr('msid_Virtualization_RMP_Efforts').AssignValue('0')
					row.Calculate()
