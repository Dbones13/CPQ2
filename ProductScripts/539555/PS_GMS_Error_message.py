...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
	...
	#Error Message for Gas_MeterSuite_Engineering_Labor_Container Labor Container
	deliverables = Product.GetContainerByName('Gas_MeterSuite_Engineering_Labor_Container').Rows
	...
	if count2 > 0:
		Labor_Message3 = "<b>"+'Cost is not available for selected resource in Gas MeterSuite Engineering - C300 Functions. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

	#Error Message for PRMS_Additional_Labor_Container
	GMS_Error_Message_val = GMS_Error_Message_val2 = ""
	deliverables = Product.GetContainerByName('Gas_MeterSuite_Additional_Labor_Container')
	count = count1 = count2 = 0
	Labor_Message3 = ''
	for row in deliverables.Rows:
		...
		if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
			count +=1
			GMS_Error_Message_val = GMS_Error_Message_val + " - " +str(deliverable) + "<br>"
		FO_Eng_1_val = row.GetColumnByName('FO Eng').DisplayValue
		finalHrs = row.GetColumnByName('Final Hrs').Value
		if row.GetColumnByName('FO Eng % Split').Value:
			FO1_Eng = float(row.GetColumnByName('FO Eng % Split').Value)
		if (finalHrs not in ('','0','0.0') and (FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
			count1 = count1 + 1
			ColumnName = "Fo Eng"
			GMS_Error_Message_val2 = GMS_Error_Message_val2 + " - " +str(deliverable) + "<br>"
	...
	if count1 > 0:
		GMS_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the {}% Split to 0% <br>'.format(ColumnName)+"</b>" + GMS_Error_Message_val2
		if Labor_Message != "":
			Labor_Message += "<br>" + GMS_Error_Message_val2
		else:
			Labor_Message = GMS_Error_Message_val2
	if count2 > 0:
	...
	if Labor_Message != '':
		Product.Attr('Error_Message').AssignValue(Labor_Message)
