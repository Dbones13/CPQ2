...
if (('Labor Deliverables' in tabs or 'General Input' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for LMS_Labor_Container Labor Container
    deliverables = Product.GetContainerByName('LMS_Labor_Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in Liquid MeterSuite Engineering - C300 Functions. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for LMS_Additional_Labor_Container
    GMS_Error_Message_val = GMS_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('LMS_Additional_Labor_Container')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    ...
    """
    if count > 0:
        GMS_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + PRMS_Error_Message_val
        if Labor_Message != "":
            Labor_Message += "<br>" + PRMS_Error_Message_val
        else:
            Labor_Message = PRMS_Error_Message_val"""

    if count1 > 0:
        GMS_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + GMS_Error_Message_val2
        if Labor_Message != "":
