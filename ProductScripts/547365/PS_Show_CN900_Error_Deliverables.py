...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #CXCPQ-24662 #CXCPQ-24672

    #Error Message for CE CN900 Engineering Labor Container
    deliverables = Product.GetContainerByName('CE CN900 Engineering Labor Container').Rows
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in CE CN900 Engineering. Please select different resource or different Execution Country. Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3
    #Error Message for CE CN900 Additional Custom Deliverables
    CN900_Error_Message_val = CN900_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('CE CN900 Additional Custom Deliverables')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        """if split != 100 and row.GetColumnByName('Final Hrs').Value not in ('','0','0.0') and deliverable != '':
            count +=1
            CN900_Error_Message_val = CN900_Error_Message_val + " - " +str(deliverable) + "<br>" """
        #CXCPQ-24662
        FO_Eng_1_val = row.GetColumnByName('FO Eng').DisplayValue
        FO1_Eng = float(row.GetColumnByName('FO Eng % Split').Value)
        finalHrs = row.GetColumnByName('Final Hrs').Value

        if (finalHrs not in ('','0','0.0') and ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0 and deliverable != '')):
            count1 = count1 + 1
            CN900_Error_Message_val2 = CN900_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    """if count > 0:
        CN900_Error_Message_val =  "<b>"+'Hours distribution among resources is not 100%. Please adjust the % Split column value to get the total 100% <br>'+"</b>" + CN900_Error_Message_val
        if Labor_Message != "":
            Labor_Message += "<br>" + CN900_Error_Message_val
        else:
            Labor_Message = CN900_Error_Message_val """

    if count1 > 0:
        CN900_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + CN900_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + CN900_Error_Message_val2
        else:
            Labor_Message = CN900_Error_Message_val2
    if count2 > 0:
