...
Release=TagParserProduct.ParseString('<*CTX ( Container(CN900_Common_Questions_Cont).Row(1).Column(CN900_Exp_PKS_software_release).Get )*>')
Quote.GetCustomField('ControlEdge_Experion_PKS_Software_Release').Content=Release

uoc_cont_rows = Product.GetContainerByName('UOC_PartSummary_Cont').Rows
...
if errorFlag:
    ...
    #Product.Messages.Clear()
    uoc_cont_rows = Product.GetContainerByName('UOC_PartSummary_Cont').Rows
    uoc_li_cont = Product.GetContainerByName('UOC_LI_PartSummary_Cont')
