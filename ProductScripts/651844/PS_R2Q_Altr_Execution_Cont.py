pole = Quote.GetCustomField('R2Q_Booking_Pole').Content
if pole == 'APAC':
	allowlist = ['India','None']
elif pole == 'EMEA':
	allowlist = ['France', 'Finland', 'Italy', 'United Kingdom','None']
elif pole == 'AMER':
	allowlist = ['United States', 'Canada','None']
else:
	allowlist = []

disallowlist = []
Prj_cont = Product.GetContainerByName("R2Q_Project_Questions_Cont")
for row in Prj_cont.Rows:
	attr_val = row.Product.Attr("R2Q_Alternate_Execution_Country").Values
	disallowlist = [i.ValueCode for i in attr_val if i.ValueCode not in allowlist]
	row.Product.DisallowAttrValues("R2Q_Alternate_Execution_Country", *disallowlist)
	alncntry_length = len(allowlist)
	if (row["R2Q_Alternate_Execution_Country"] != Quote.GetCustomField("R2Q_Alternate_Execution_Country").Content or row["R2Q_Alternate_Execution_Country"] == '') and alncntry_length != 2:
		row["R2Q_Alternate_Execution_Country"] = "None"
		row.Product.Attributes.GetByName('R2Q_Alternate_Execution_Country').SelectDisplayValue("None")
	else:
		if alncntry_length == 2 and allowlist[1] == 'None':
			row["R2Q_Alternate_Execution_Country"] = allowlist[0]
			row.Product.Attributes.GetByName('R2Q_Alternate_Execution_Country').SelectDisplayValue(allowlist[0])
		else:
			row["R2Q_Alternate_Execution_Country"] = Quote.GetCustomField("R2Q_Alternate_Execution_Country").Content
	#Log.Info("R2Q_Alternate_Execution_Country == "+str(row["R2Q_Alternate_Execution_Country"])+"CF R2Q_Alternate_Execution_Country -- "+str(Quote.GetCustomField("R2Q_Alternate_Execution_Country").Content))

Product.DisallowAttrValues("R2Q_Alternate_Execution_Country", *disallowlist)

if Product.Attr('Sell Price Strategy').SelectedValue.Display == 'Customer Budget':
	Product.AllowAttr('Customer_Budget_TextField')
	Product.AllowAttr('Customer_Budget_USD')
	Product.Attr('Customer_Budget_USD').Access = AttributeAccess.ReadOnly
else:
	Product.DisallowAttr('Customer_Budget_TextField')
	Product.DisallowAttr('Customer_Budget_USD')
cont = Product.GetContainerByName('R2Q_Project_Questions_Cont')
if Quote.GetCustomField("R2Q_Alternate_Execution_Country").Content =='':
    for i in cont.Rows:
        Quote.GetCustomField("R2Q_Alternate_Execution_Country").Content = i['R2Q_Alternate_Execution_Country']