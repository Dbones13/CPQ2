...
for item in Product.GetContainerByName('SM_RG_ATEX Compliance_and_Enclosure_Type_Cont').Rows:
    if item['Enclosure_Type'] == 'Cabinet' and Product.GetContainerByName('SM_SC_Universal_Safety_Cab_1_3M_Details_cont'):
        for item in Product.GetContainerByName('SM_SC_Universal_Safety_Cab_1_3M_Details_cont').Rows:
            item['PUIO_Count'] = ''
            item['PDIO_Count'] = ''
    elif item['Enclosure_Type'] == 'Universal Safety Cab-1.3M':
        for item in Product.GetContainerByName('SM_SC_Universal_Safety_Cab_1_3M_Details_cont').Rows:
            if item['PUIO_Count'] == '':
                item.GetColumnByName('PUIO_Count').SetAttributeValue('64')
            if item['PDIO_Count'] == '':
                item.GetColumnByName('PDIO_Count').SetAttributeValue('0')