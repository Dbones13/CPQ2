...
for attr in attrs:
    ...
    if container.Rows.Count == 0:
        ...
        if attr == 'SM_RG_ATEX Compliance_and_Enclosure_Type_Cont':
            ...
            row.GetColumnByName('SM_RG_RelayTypeForESD').SetAttributeValue('Non SIL')
            ...
            row.GetColumnByName('S300').SetAttributeValue('No S300')
            row.GetColumnByName('S300').ReferencingAttribute.SelectValue('X')
            row.GetColumnByName('S300').ReferencingAttribute.SelectValue('No S300')
            row['S300'] = 'X'
