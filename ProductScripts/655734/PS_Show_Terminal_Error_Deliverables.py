...
if (('Labor Deliverables' in tabs or 'General Inputs' in tabs) and scope == 'HW/SW + LABOR'):
    ...
    #Error Message for Terminal Engineering Labor Container
    deliverables = Product.GetContainerByName('Terminal Engineering Labor Container').Rows
    gesLocation = gesLocation = Product.Attr("Terminal_Ges_Location_Labour").GetValue()
    ...
    if count2 > 0:
        Labor_Message3 = "<b>"+'Cost is not available for selected resource in Terminal Engineering Labor Container. Please select different resource or different Execution Country.Deliverables to look into: <br>'+"</b>" + Labor_Message3
        if Labor_Message != "":
            Labor_Message += "<br>" + Labor_Message3
        else:
            Labor_Message = Labor_Message3

    #Error Message for Terminal Additional Custom Deliverables
    Terminal_Error_Message_val = Terminal_Error_Message_val2 = ""
    deliverables = Product.GetContainerByName('Terminal Additional Custom Deliverables')
    count = count1 = count2 = 0
    Labor_Message3 = ''
    for row in deliverables.Rows:
        ...
        FO1_Eng = float(row.GetColumnByName('FO Eng % Split').Value)
        finalHrs = row.GetColumnByName('Final Hrs').Value
        if ((FO_Eng_1_val == '' or FO_Eng_1_val == 'None') and FO1_Eng_New > 0):
            count1 = count1 + 1
            Terminal_Error_Message_val2 = Terminal_Error_Message_val2 + " - " +str(deliverable) + "<br>"
    ...
    if count1 > 0:
        Terminal_Error_Message_val2 = "<b>"+'Resource is not selected for deliverable. Please select respective Eng or change the Eng % Split to 0% <br>'+"</b>" + Terminal_Error_Message_val2
        if Labor_Message != "":
            Labor_Message += "<br>" + Terminal_Error_Message_val2
        else:
            Labor_Message = Terminal_Error_Message_val2
    if count2 > 0:
