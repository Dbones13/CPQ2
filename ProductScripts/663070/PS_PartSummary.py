part_summary = Product.Tabs.GetByName('Part Summary').IsSelected
if part_summary:
...
if Product.Attr('PERF_ExecuteScripts').GetValue() != '':
	...
	if Exp_UOC_BP > 0:
		...
		Qty_of_1000, Qty_of_100, Qty_of_10, apt_qty_of_100 = UOC_Parts.part_qty_1000_50(Exp_UOC_BP)
		if Is_TPS == 'No':
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-1BP01K",int(Qty_of_1000))
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-1BP100",int(Qty_of_100))
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-1UOS50",int(Qty_of_10))
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","ES-1UOS50",0)
		elif Is_TPS == 'Yes':
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-1BP01K",int(Qty_of_1000))
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-1BP100",int(Qty_of_100))
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","ES-1UOS50",int(Qty_of_10))
			GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-1UOS50",0)
	else:
		GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-1BP01K",0)
	...
	if Exp_UOC_ABP > 0:
		...
		Qty_of_1000, Qty_of_100, Qty_of_10, apt_qty_of_100 = UOC_Parts.part_qty_1000_50(Exp_UOC_ABP)
		GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-2BP01K",int(Qty_of_1000))
		GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-2BP100",int(apt_qty_of_100))
	else:
		GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","EP-2BP01K",0)
	...
	#TagParserProduct.ParseString("""<*CTX( Container(Exp_Ent_Grp_Part_Summary_Cont).LoadKeys(PartNumber).Load(select '<*Eval(Replace("<*VALUE(Exp_Ent_Grp_Part_Summary)*>",", ","' as PartNumber UNION SELECT '"))*>' as PartNumber   ) )*>""")
	Exp_Ent_Grp_Part_Summary = Product.Attr('Exp_Ent_Grp_Part_Summary').GetValue()
	if Exp_Ent_Grp_Part_Summary != '':
		...
		Product.GetContainerByName('Exp_Ent_Grp_Part_Summary_Cont').LoadFromDatabase(Exp_Ent_Grp_Part_Summary_Query, 'PartNumber')

	Product.ApplyRules()
	Product.ExecuteRulesOnce = False
	Product.Attr('PERF_ExecuteScripts').AssignValue('')
	Product.ParseString('<* ExecuteScript(HIde Cluster Cabinet Mounting Station) *><* ExecuteScript(PS_Cab_desk_defaults) *><* ExecuteScript(PS_CXDEV-7712) *><* ExecuteScript(DMS HIde Cluster Cabinet Mounting Station) *><* ExecuteScript(PS_Cabinet_Display_Size) *>')