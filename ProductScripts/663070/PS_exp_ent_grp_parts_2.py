if Product.Attr('PERF_ExecuteScripts').GetValue() != '':
    ...
    if ESR == 'R530':
        ...
        GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","MZ-PCWS15",0)
    #Commenting below code because of duplicate Partnumber logic it is already present in "PS_exp_ent_grp_parts" script
    """if ESR == 'R530':
    ...
    if ESR == 'R520':
        MZ_PCWT02_qty = GS_BOM_exp_ent_grp.station_qnt1(Product,'','STN_PER_DELL_Tower_RAID2')
        if MZ_PCWT02_qty > 0:
            GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","MZ-PCWT02",MZ_PCWT02_qty)
        else:
            GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","MZ-PCWT02",0)
    else:
        GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","MZ-PCWT02",0)
    ...
    # CXCPQ-116381
    '''qt_CF_CTD001 = GS_BOM_exp_ent_grp_2.total_servers_and_stations(Product)
    if qt_CF_CTD001>0:
        Trace.Write("38883 CF-CTD001")
        GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","CF-CTD001",qt_CF_CTD001)
    else:
        GS_PS_Exp_Ent_BOM.setAtvQty(Product,"Exp_Ent_Grp_Part_Summary","CF-CTD001",0)'''
